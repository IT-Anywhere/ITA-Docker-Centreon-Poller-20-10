...
"iRED", "ired" - iRMC Redfish S5 or higher
...
our $optGetStorageTree = undef;
our $optReadJson = undef;
...
#----------- miscelaneous helpers 
  ...
  sub addTmpExitCode {
	...
	my $tmpCode = shift;
	if (defined $addCode) {
		if (!defined $tmpCode or $tmpCode == 3) {
			$tmpCode = $addCode;
		} else {
...
#----------- performance data functions
  ...
  sub addPowerConsumptionToPerfdata {
	...
	if (defined $current) {
		...
		$performanceData .= " PowerConsumption=0" . $unit;
	}
...
#----------- output format functions -for special items
 ...
 sub addKeyVolt {
  ...
  sub addKeyMHz {
	...
	my $speed = shift;
	my $tmp = '';
	$speed = undef if (defined $speed and $speed =~ m/[-]*\d+/ and $speed == -1);
...
#### OPTION FUNCTION ######
  ...
  sub setOptions {	# script specific
	...
	$k="authmode";	$optAuthMode = $options{$k}		if (defined $options{$k});
	# pure REST specific
	...
	$k="getstoragetree";    $optGetStorageTree = $options{$k} if (defined $options{$k});
	$k="json";		$optReadJson = $options{$k}	if (defined $options{$k});
  ...
  sub evaluateOptions {	# script specific
	...
	pod2usage(
		...
		-verbose	=> 0,
		-exitval	=> 3
	) if (!$optReadJson and (!$optHost or $optHost eq '') and (!$optAdminHost or $optAdminHost eq ''));
...
our $useRESTverbose = 0;
  ...
  sub restCall {
	...
	{   $cmd = "curl --stderr - -s -S";
	    ...
	    #$cmd .= " --anyauth" if ($hasiRMCS5Redfish);
		# --anyauth: --basic,  --digest,  --ntlm,  and  --negotiate
	    if ($optAuthMode) {
		$cmd .= " --anyauth" if ($optAuthMode =~ m/any/i);
		$cmd .= " --basic" if ($optAuthMode =~ m/basic/i);
		$cmd .= " --digest" if ($optAuthMode =~ m/digest/i);
		$cmd .= " --ntlm" if ($optAuthMode =~ m/ntlm/i);
		$cmd .= " --negotiate" if ($optAuthMode =~ m/negotiate/i);
	    }
	    $cmd .= " $addCurlParam" if ($addCurlParam);
	    $cmd .= " --connect-timeout $optConnectTimeout" if ($optConnectTimeout);
	    $cmd .= " --max-time $optConnectTimeout" if ($optConnectTimeout);
	    $cmd .= " --max-time $optTimeout" if (!$optConnectTimeout and $optTimeout);
	    $cmd .= " --max-time $optDefaultTimeout" if (!$optConnectTimeout and !$optTimeout and $optDefaultTimeout);
	...
	if ($outPayload) {
	    $outPayload =~ s/\{\s+\[data not shown\]\n//gm;
	    $outPayload =~ s/\{\s+\[data not shown\]//gm;
	    $outPayload =~ s/\{\s+\[\d+ bytes data\]\n//gm;
	    $outPayload =~ s/\{\s+\[\d+ bytes data\]//gm;
	    $outPayload =~ s/\}\s+\[\d+ bytes data\]\n//gm;
	    $outPayload =~ s/\}\s+\[\d+ bytes data\]//gm;
	    $outPayload =~ s/\* SSLv\d, Unknown \(\d+\):\n//gm;
	    $outPayload =~ s/\* SSLv\d, Unknown \(\d+\)://gm;
	    $outPayload =~ s/\s\sCApath: [^\s]+\n//gm;
	}
	...
	} elsif ($main::verbose >= 20 and $outPayload) {
	    if ($main::verbose < 60) {
		print "**** RESPONSE: \n$outPayload\n";
	    } else {
		print "**** RESPONSE: \n... see output above\n";
...
#########################################################################
  ...
  sub jsonSplitObjectList {
	...
	while ($response and $response !~ m/^\s*}/) {
		...
		$jsonIndent="" if (! defined $jsonIndent);
		print "OBJECTLIST$jsonIndent-$key-$type-$value-\n" if ($main::verbose >= 30 and $key);
	    }
  ...
  sub jsonSplitResponse { # expects only one main object !
	...
	return undef if ($response !~ m/^\s*{/m);
	return undef if ($response !~ m/}\s*$/m);
  ...
  sub jsonGetSubKeyValue {
	...
	foreach my $refKeyValue ( @list) {
	    ...
	    if ($key =~ m/__exact$/) {
		my $refkey = $refKeyValue->{"KEY"};
		my $parkey = $key;
		$parkey =~ s/__exact//;
		return $refKeyValue if ($refkey eq $parkey);
	    } else {
...
#########################################################################
 ...
 ####
  sub agentPrintOneFan {
	...
	$medium = "l" if (!$medium and $notify and $status > 1 and $status < 5);
	if ($medium) {
	    ...
	    if ($#maxvalues >= 1 and (($status > 1 and $status < 5) or ($verbose and $main::verbose >= 3) )) {
		addKeyRpm($medium,"MaxTestSpeed", $maxvalues[0]);
		addKeyRpm($medium,"NominalTestSpeed", $maxvalues[1]);
	    }
	    addMessage($medium,"\n");
	} 
 ...
 ####
  ...
  sub agentRAIDCtrlTable {
	...
	my @typeText = ( "SCSI",
	    ...
	    "SD card bus", "AHCI", "PCIE", "LMD", "NVMe", 
	    "..unexpected..",
	); # + 0xFF "Uknown"
	...
	for (my $i=0;$i<$gAgentRAIDCtrlNumber;$i++) {
	    ...
	    $typenr = 16 if (!defined $typenr or $typenr !~ m/^\d+$/ 
		or $typenr < 0 or ($typenr > 15 and $typenr != 255));
	    my $type = undef;
  ...
  sub agentRAIDPhysicalDeviceTable {
	...
	my $storageDeviceTree = agentGetStorageTree();
	my %pdLifeTime = ();
	if ($storageDeviceTree and $storageDeviceTree =~ /EstimatedLifetime/im) {
	    %pdLifeTime = readJsonStorageDeviceTree($storageDeviceTree);	    
	}
	my $refCmd = agentJson_ExtractCmd($providerout);
	for (my $c=0;$c<$gAgentRAIDCtrlNumber;$c++) {
	    for (my $i=0;$i<$gAgentRAIDPDeviceNumber{$c};$i++) {
		...
		if ($medium) {
			...
			addKeyLongValue($medium,"Property", $prop);
			if (%pdLifeTime and ($pdLifeTime{"$c.$i"} or $pdLifeTime{"$c.$name"})) {
			    my $esttime = $pdLifeTime{"$c.$i"};
			    $esttime = $pdLifeTime{"$c.$name"} if (!$esttime);
			    addKeyLongValue($medium,"EstimatedLifetime", $esttime);
			}
			addMessage($medium,"\n");
		}
	    } # for 
 ...
 ####
  ...
  sub agentGetStorageTree {
	{ # nr of Update Components
	    #### BUILD
	    (my $refMain, my $refArrayCmd) = agentJson_CreateJsonCmd();  
	    agentJson_AddCmd($refArrayCmd,"RaidStorageDeviceTree");
	    #### CALL REST/JSON
	    (my $rc, my $providerout) = agentJson_CallCmd($refMain);
	    return if ($rc == 2);
	    #### SPLIT
	    my $refCmd = agentJson_ExtractCmd($providerout);
	    my $storagetree = agentJson_GetCmdSimpleData($refCmd,"RaidStorageDeviceTree"); 
	    return $storagetree;
	}
  } # agentGetStorageTree
    sub agentStripNonJsonParts {
	my $out = shift;
	$out =~ s/@//gm;
	$out =~ s/\[\s+\]/\[\]/gm;		# strip empty arrays
	$out =~ s/\{\s+\}/\{\}/gm;		# strip empty objects
	$out =~ s/\:null/\:\"null\"/gm;		# replace Redfish-null to String
	$out =~ s/\[\]/\"null\"/gm;		# mark empty arrays
	$out =~ s/\{\}/\"null\"/gm;		# mark empty objects
	return $out;
    } #
  sub readJsonStorageDeviceTree {
      my $stream = shift;
      my $buffer = undef;
      my %info = ();
      $buffer = readDataFile($optReadJson) if ($optReadJson and !$stream);
      $buffer = $stream if ($stream);
      $buffer = agentStripNonJsonParts($buffer);
      my $refoutMain = undef;  
      $refoutMain = jsonSplitResponse($buffer);
      my $treeobj = jsonGetSubKeyValue($refoutMain, "SVRaidStorageDeviceTree");
      if (!$treeobj) {
	  print "ERROR: no SVRaidStorageDeviceTree\n" if ($optReadJson);
	  return;
      } 
      my $refsystem = jsonGetSubKeyValue($treeobj, "System");
      if (!$refsystem) {
	  print "ERROR: no System\n" if ($optReadJson);
	  return;
      } 
      my $refadapterArray = jsonGetSubKeyValue($refsystem, "Adapter");
      if (!$refadapterArray) {
	  print "ERROR: no Adapter array\n" if ($optReadJson);
	  return;
      } 
      my $type = $refadapterArray->{"TYPE"};
      return if ($type ne "ARRAY"); # null
      my @adapters = jsonGetArrayList($refadapterArray);
      return if ($#adapters < 0);
      my $anr = undef;
      foreach my $oneRefAdapter (@adapters) {
	  my $refAdpIndex = jsonGetSubKeyValue($oneRefAdapter, "Nr");
	  my $refAdpNr = jsonGetSubKeyValue($oneRefAdapter, "AdapterNumber");
	  my $refAdpName = jsonGetSubKeyValue($oneRefAdapter, "Name");
	  $anr = $refAdpNr->{"VALUE"};
	  print "Adapter " . $refAdpIndex->{"VALUE"} . ' ' . $refAdpNr->{"VALUE"} . ' ' . $refAdpName->{"VALUE"} . "\n" if ($optReadJson);
	  #### flat PDs
	  my $refPDs = jsonGetSubKeyValue($oneRefAdapter, "PhysicalDrive");
	  if (!$refPDs) { 
	      print "---- no PDs\n" if ($optReadJson); 
	  } else {
	      $type = $refPDs->{"TYPE"};
	      if ($type eq "ARRAY") {
		  my @pds = jsonGetArrayList($refPDs);
		  # next if ($#pds < 0);
		  foreach my $refOnePD (@pds) {
		      my $refLifeTime = jsonGetSubKeyValue($refOnePD, "EstimatedLifetime");
		      next if (!$refLifeTime);
		      my $refDevNr =  jsonGetSubKeyValue($refOnePD, "DeviceNumber");
		      my $dnr = undef;
		      $dnr = $refDevNr->{"VALUE"} if ($refDevNr);
		      my $refPortNr =  jsonGetSubKeyValue($refOnePD, "PortNumber");
		      my $pnr = $refPortNr->{"VALUE"} if ($refPortNr);
		      my $refSlotNr =  jsonGetSubKeyValue($refOnePD, "Slot");
		      my $snr = $refSlotNr->{"VALUE"} if ($refSlotNr);
		      my $lifeTime = $refLifeTime->{"VALUE"} ;
		      my $keyValue="$anr.$dnr" if (defined $dnr);
		      my $refIndex = jsonGetSubKeyValue($refOnePD, "Nr");
		      my $refName = jsonGetSubKeyValue($refOnePD, "Name");
		      my $name = $refName->{"VALUE"} if ($refName);
		      $dnr = $refIndex->{"VALUE"} if (!defined $dnr && $refIndex);
		      my $msg = "**** EstimatedLifetime PD Index#=" . $refIndex->{"VALUE"};
		      $msg .= " Device#=$dnr" if (defined $dnr);
		      $msg .= " Port#=$pnr" if (defined $pnr);
		      $msg .= " Slot#=$snr" if (defined $snr);
		      $msg .= " Name=\"$name\"" if (defined $name);
		      $msg .= " Adapter#=$anr Time=$lifeTime \n";
		      print $msg if ($optReadJson or $main::verbose >= 20);
		      $info{"$anr.$dnr"} = $lifeTime if (defined $dnr);
		      $info{"$anr.$name"} = $lifeTime if (defined $name);
		  }
	      } # array
	  }
	  #### Ports  
	  my $refPorts = jsonGetSubKeyValue($oneRefAdapter, "Port__exact"); # needs __exact because port is part of other keys
	  if (!$refPorts) { 
	      print "---- no Ports\n" if ($optReadJson); 
	      next;
	  } else { 
	      ...
	      $type = $refPorts->{"TYPE"};
	      if ($type eq "ARRAY") {
		  my @ports = jsonGetArrayList($refPorts);
		  foreach my $refOnePort (@ports) {
		      print "Port:\n" if ($optReadJson);
		      my $refPDs =  jsonGetSubKeyValue($refOnePort, "PhysicalDrive");
		      if (!$refPDs) { 
			  print "---- no PDs\n" if ($optReadJson); 
		      } else {
			  $type = $refPDs->{"TYPE"};
 ...
 ####
  ...
  sub iRMCReportAllMemoryModules {
	...
	for (my $i=0;$i <= $#sensorArray; $i++) {
	    ...
	    $config = $1	if ($sensor =~ m/ConfigStatus Description=\"([^\"]+)\"/);
	    #
	    $status = 0 if (!defined $status);
	    #
	    ...
	    if ($medium) {
		    ...
		    addKeyMHz($medium,"Frequency-Max", $max);
		    addKeyValue($medium,"Voltage", $volt);
		    addKeyLongValue($medium,"Configuration", $config);
		    addMessage($medium,"\n");
	    }
	    if ($setExitCode) {
		my $localExitCode = 3;
...
#########################################################################
  ...
  sub iRMCRestConnectionTest { 
	...
	#
	if ($out and $out =~ m/^\s*\{/m) { # must be a JSON answer
...
#########################################################################
  ...
  sub iRMCRedfishConnectionTest { 
	...
	my $mustwait = 0;
	my $webserver = undef;
	$webserver = $1 if ($outheader and $outheader =~ m/Server:\s+(iRMC[^\n]+)/m);
	if ($out and $out =~ m/^\s*\{/m) { # must be a JSON answer
	    $optServiceType = "iRed" if ($out and !$optServiceType);
	    $hasiRMCS5Redfish = 1 if ($webserver and $webserver =~ m/S[5-9]/);
	    $isiRMCS5 = 1 if ($webserver and $webserver =~ m/S[5-9]/);
	    addExitCode(0);
	} elsif ($outheader and $outheader =~ m/ 503 /) {
	    $optServiceType = "iRedWAIT" if ($out and !$optServiceType);
	    $mustwait = 1;
	    addExitCode(0);
	    $hasiRMCS5Redfish = 0 if ($webserver and $webserver =~ m/S[5-9]/);
	}
	if ($out and $out =~ m/ServerView Remote Management/) {
	    addMessage("l","[ERROR] Detected iRMC - but no Redfish service available\n");
	    addExitCode(1); # prevent more iRMC checks
	}
	if ($exitCode == 0 and $out and !$hasiRMCS5Redfish) {
	    my $outV1 = redfishGET("/redfish/v1");
	    my $redfishVersion = undef;
	    $outV1 =~ s/\n//gm if ($outV1);
	    $redfishVersion = $1 if ($outV1 and $outV1 =~ m/\"RedfishVersion\"\s*\:\s*\"([^\"]*)\"/m);
	    if ($redfishVersion) {
		$hasiRMCS5Redfish = 1 if ($redfishVersion =~ m/1\.0\.[4-9]/);
		$hasiRMCS5Redfish = 1 if (!$hasiRMCS5Redfish and $redfishVersion gt "1.0.5");
		$useRedfishExpand = 1 if ($hasiRMCS5Redfish and $main::verboseTable != 100);
	    }
	    #$exitCode = 3 if (!$hasiRMCS5Redfish);
	}
	addExitCode(0) if ($hasiRMCS5Redfish);
	# default:
	$optServiceType = "iRed" if ($hasiRMCS5Redfish);
	if ($exitCode == 0 and $optChkIdentify and $hasiRMCS5Redfish) {
	    addMessage("m","- ") if (!$msg);
	    addKeyLongValue("m","Redfish-Service", "ServerView iRMC Redfish") if (!$mustwait);
	    addKeyLongValue("m","Redfish-Service", "ServerView iRMC Redfish (Uninitialized)") if ($mustwait);
	} elsif ($webserver and $out and !$hasiRMCS5Redfish and (!$optServiceType or $optServiceType =~ m/^ired/i)) {
	    addMessage("m","- ") if (!$msg);
	    addMessage("m","[ERROR] Detected iRMC - but no valid Redfish Service available");
	}
	$optRestHeaderLines = undef;
	$optConnectTimeout = $save_optConnectTimeout;
  } # iRMCRedfishConnectionTest

  sub redfishExtractLink {
	my $key = shift;
	my $out = shift;
	my $link = undef;
	$link = $1 if ($out =~ m/\"$key\"\:\{([^\}]+)\}/m);
	$link =~ s/^[^:]*://m if ($link);
	$link =~ s/^\"//m if ($link);
	$link =~ s/\"\s*$//m if ($link);
	$link =~ s!\\/!/!g if ($link);
  ...
  sub redfishExtractFirstArrayLink {
	my $out = shift;
	my $link = undef;
	$link = $1 if ($out =~ m/\"Members\"\:\s*\[\s*\{\s*([^\}]+)\s*\}\s*\]/m);
	$link =~ s/^[^:]*://m if ($link);
	$link =~ s/^\"//m if ($link);
	$link =~ s/\"\s*$//m if ($link);
	$link =~ s!\\/!/!g if ($link);
	return $link;
  } # redfishExtractFirstArrayLink
  sub redfishArrayLinks {
	my $out = shift;
	my @outArray = ();
	return \@outArray if (!$out);
	my $members = $1 if ($out =~ m/\"Members\"\s*\:\s*\[([^\]]*)\]/m);
	return \@outArray if (!$members);
	$members =~ s/\n//gm;
	while ($members) {
	    my $link = undef;
	    $link = $1 if ($members =~ m/\s*[,]*\{([^\}]+)/);
	    $link =~ s/.*\:\s*\"// if ($link);
	    $link =~ s/\"\s*$// if ($link);
	    $link =~ s!\\/!/!g if ($link);
	    push (@outArray, $link);
  ...
  sub redfishGET {
	...
	$optRestHeaderLines = "Accept: application/json";
	(my $out, my $outheader, my $errtext) = 
		restCall("GET", $url, undef);
	#
	return $out;
  } # redfishGET
  our %redfishCodeText = ( 0 => "OK", 1 => "Warning", 2 => "Critical", 3 => "NotAvailable");
  sub redfish_code {
	my $text = shift;
	return 3 if (!$text);
	return 0 if ($text eq "OK");
	return 1 if ($text eq "Warning");
	return 2 if ($text eq "Critical");
	return 3;
  } # redfish_code
  sub redfish_simpleKeyString {
	my $key = shift;
	my $stream = shift;
	return undef if (!$key or !$stream);
      	my $out = undef;
	$out = $1 if ($stream =~ m/\"$key\"\s*\:\s*\"([^\"]*)\"/m);
	return $out;
  } # redfish_simpleKeyString
  sub redfish_simpleKeyNumber {
	my $key = shift;
	my $stream = shift;
	return undef if (!$key or !$stream);
      	my $out = undef;
	$out = $1 if ($stream =~ m/\"$key\"\s*\:\s*([\d.]*)/m);
	return $out;
  } # redfish_simpleKeyNumber
 
  our $redfishChassis0 = undef;
  our $redfishChassis0Response = undef;
  our $redfishSystem0 = undef;
  our $redfishSystem0Response = undef;
  our $redfish0ThermalResponse = undef;
  our $redfish0PowerResponse = undef;
  our $redfish0StorageResponse = undef;
  our $redfishManagersiRMCResponse = undef;
  our $redfishManagersiRMCConfigurationSystemResponse = undef;
  our $refRedfishChassis = undef; # reference on array - UNUSED
  our $refRedfishChassisResponse = undef; # reference on array - UNUSED
  sub redfishSystem0Link {
	my $systemsLink = shift;
	return if (!$hasiRMCS5Redfish);
	#my $out = redfishGET($systemsLink);
	$optRestHeaderLines = "Accept: application/json";
	$useRESTverbose = 1;
	(my $out, my $outheader, my $errtext) = 
		restCall("GET", $systemsLink, undef);
	if ($outheader and $outheader =~ m/HTTP.[\d\.]+ 401/) {
	    #addMessage("m","- ") if (!$msg);
	...
	$useRESTverbose = 0;
	if (!$out or $out eq "" or $out !~ m/^\s*\{/m) { # must be a JSON answer
	    if ($optServiceType =~ m/^ired/i) {
		addExitCode(2);
	    	#addMessage("m","- ") if (!$msg);
		addMessage("m","\n[ERROR] Unable to read $systemsLink\n");
	    }
	    return; 
	}
	my $count = 0;
	$count = $1 if ($out =~ m/\"Members\@odata.count\"\:\s*(\d+)/m);
	if ($count != 1) {
	    if ($optServiceType =~ m/^ired/i) {
		addExitCode(2);
	    	#addMessage("m","- ") if (!$msg);
		addMessage("m","\n[ERROR] Unexpected count of systems in $systemsLink\n");
	    }
	    return; 
	}
	my $link = redfishExtractFirstArrayLink($out);
	return $link;
  } # redfishSystem0Link
  sub redfishChassis0Link {
      my $chassisLink = shift;
      return if (!$hasiRMCS5Redfish);
	$optRestHeaderLines = "Accept: application/json";
	my $out = redfishGET($chassisLink);
	if (!$out or $out !~ m/^\s*\{/m) { # must be a JSON answer
	    if ($optServiceType =~ m/^ired/i) {
		addExitCode(2);
	    	addMessage("m","- ") if (!$msg);
		addMessage("m","[ERROR] Unable to read $chassisLink\n");
	    }
	    return; 
	}
	$refRedfishChassis = redfishArrayLinks($out);
	my $link = redfishExtractFirstArrayLink($out);
	return $link;
  } # redfishChassis0Link
  sub redfishRootLinks { # called by connectionTest
	return if (!$hasiRMCS5Redfish);
	my $out = redfishGET("/redfish/v1");
	if (!$out or $out !~ m/^\s*\{/m) { # must be a JSON answer
	    if ($optServiceType =~ m/^ired/i) {
		addExitCode(2);
	    	addMessage("m","- ") if (!$msg);
		addMessage("m","[ERROR] Unable to read /redfish/v1\n");
	    }
	    return; 
	}
	my $redfishSystems = redfishExtractLink("Systems", $out);
	my $redfishChassis = redfishExtractLink("Chassis", $out);	
	$redfishSystem0 = redfishSystem0Link($redfishSystems);
	$redfishChassis0 = redfishChassis0Link($redfishChassis) if ($exitCode != 2);
  } # redfishRootLinks
  sub redfishSystem0Response {
	return if ($redfishSystem0Response);
	return if (!$redfishSystem0);
	my $out = redfishGET($redfishSystem0);
	$redfishSystem0Response = $out;
	if (!$out or $out !~ m/^\s*\{/m) { # must be a JSON answer
	    if ($optServiceType =~ m/^ired/i) {
		addExitCode(2);
		addMessage("l","[ERROR] Unable to read /redfish/v1 and System data\n");
	    }
	    return; 
	}
  } # redfishSystem0Response
  sub redfishChassis0Response {
	if (!$redfishChassis0Response and $redfishChassis0) {
	    my $out = redfishGET($redfishChassis0);
	    $redfishChassis0Response = $out;
	    if (!$out or $out !~ m/^\s*\{/m) { # must be a JSON answer
		if ($optServiceType =~ m/^ired/i) {
		    addExitCode(2);
		    addMessage("l","[ERROR] Unable to read /redfish/v1 and Chassis data\n");
		}
		return; 
	    }
	}
  } # redfishChassis0Response
  sub redfishChassisResponse { # all none-0 chassis - UNUSED
	return if (!$hasiRMCS5Redfish);
	return if (!$redfishChassis0);
	return if (!$refRedfishChassis);
	return if ($refRedfishChassisResponse);
	redfishChassis0Response();
	push(@{$refRedfishChassisResponse}, $redfishChassis0Response);
	my $i=0;
	foreach my $oneLink (@{$refRedfishChassis}) {
	    if (!$i) {
		$i++;
		next;
	    }
	    my $out = redfishGET($oneLink);
	    push(@{$refRedfishChassisResponse}, $out);
	} # foreach
  } # redfishChassisResponse
  sub redfishManagersiRMCResponse {
	return if ($redfishManagersiRMCResponse);
	$optRestHeaderLines = "Accept: application/json";
	my $out = redfishGET("/redfish/v1/Managers/iRMC");
	$redfishManagersiRMCResponse = $out;
  } # redfishManagersiRMCResponse
  sub redfishManagersiRMCConfigurationSystemResponse {
	return if ($redfishManagersiRMCConfigurationSystemResponse);
	# /Oem/ts_fujitsu/System for newer Versions
	my $out = redfishGET($redfishSystem0 . "/Oem/ts_fujitsu/System");
	    my $systemName = redfish_simpleKeyString("SystemName",$out);
	    if ($systemName) {
		$redfishManagersiRMCConfigurationSystemResponse = $out;
		return;
	    }
	#
	redfishManagersiRMCResponse();
	if ($redfishManagersiRMCResponse) {
	    my $confLink = redfishExtractLink("iRMCConfiguration", $redfishManagersiRMCResponse);
	    $redfishManagersiRMCConfigurationSystemResponse = redfishGET($confLink . "/System");
	}
  } # redfishManagersiRMCConfigurationSystemResponse
  sub redfish0ThermalResponse {
	return if (!$hasiRMCS5Redfish);
	return if ($redfish0ThermalResponse);
	redfishChassis0Response();
	#redfishChassisResponse();
	my $redfish0Thermal = undef;
	if ($redfishChassis0Response) {
	    $redfish0Thermal = redfishExtractLink("Thermal", $redfishChassis0Response);
	}
	return if (!$redfish0Thermal);
	my $out = redfishGET($redfish0Thermal);
	$redfish0ThermalResponse = $out
  } # redfish0ThermalResponse
  sub redfish0PowerResponse {
	return if (!$hasiRMCS5Redfish);
	return if ($redfish0PowerResponse);
	redfishChassis0Response();
	my $redfish0Power = undef;
	if ($redfishChassis0Response) {
	    $redfish0Power = redfishExtractLink("Power", $redfishChassis0Response);
	}
	return if (!$redfish0Power);
	my $out = redfishGET($redfish0Power);
	$redfish0PowerResponse = $out
  } # redfish0PowerResponse
  sub redfishSysStoragesResponse {
	return if (!$hasiRMCS5Redfish);
	return $redfish0StorageResponse if ($redfish0StorageResponse);
	redfishSystem0Response();
	return if (!$redfishSystem0Response);
	my $redfishStorages = redfishExtractLink("Storage", $redfishSystem0Response);
	return if (!$redfishStorages);
	$redfish0StorageResponse = redfishGET($redfishStorages);
	return $redfish0StorageResponse;
  } # redfishSysStoragesResponse
  ####
  our $redfishFWVersion = undef;
  sub redfishFWVersion {
	return if ($redfishFWVersion);
	redfishManagersiRMCResponse();
	my $out = $redfishManagersiRMCResponse;
	$redfishFWVersion = redfish_simpleKeyString("FirmwareVersion",$out);
  } # redfishFWVersion
  sub redfishSerialID {
	return if (!$hasiRMCS5Redfish or !$redfishSystem0);
	redfishSystem0Response();
	# hack for easier access
	my $out = $redfishSystem0Response;
	$out =~ s/\[[^\[\]]+\]//gm; # hack for easier access
	$out =~ s/\n//gm;
	$out =~ s/.*AssetTag//m; # ... The SerialNumber after AssetTag is searched
	my $ser = redfish_simpleKeyString("SerialNumber", $out);
	redfishFWVersion() if ($isiRMCS5);
	if ($isiRMCS5 and $redfishFWVersion and $redfishFWVersion ge "1.16") {
	    $useRedfishExpand = 1;
	}
	#
	return $ser;
  } # redfishSerialID
  sub redfishSystemInventoryInfo {
	return if (!$hasiRMCS5Redfish);
	my $out=undef;
	my $outheader=undef;
	my $errtext=undef;
	if ($optServiceType and $optServiceType =~ m/^ired/i) {
	    redfishManagersiRMCResponse();
	    my $out = $redfishManagersiRMCResponse;
	    my $vers = redfish_simpleKeyString("FirmwareVersion",$out);
	    #my $connectagt = undef;
	    my $agtversion = undef;
	    my $manag = undef;
	    $manag = $1 if ($out and $out =~ m/\"Model\"\s*\:\s*\"(iRMC[^\"]+)\"/m);
	    redfishManagersiRMCConfigurationSystemResponse();
	    if ($redfishManagersiRMCConfigurationSystemResponse) {
		$agtversion = redfish_simpleKeyString("AgentVersion",$redfishManagersiRMCConfigurationSystemResponse);
	    }
	    my $medium = undef;
	    if ($out and $optAgentInfo) {
		addKeyValue("m","Version",$vers);
		$medium = "l";
		$exitCode = 0;
	    } elsif ($out and $main::verbose >= 3) {
		$medium = "v";
	    }
	    if ($medium) {
		addStatusTopic($medium,undef,"AgentInfo", undef);
		addName($medium,$manag);
		addKeyValue($medium,"Version",$vers);
		addKeyValue($medium,"ConnectedAgentVersion", $agtversion);
		addMessage($medium,"\n");
	    }
	} # only pure iRMC S5
	if ($main::verbose >= 3 and !$optAgentInfo) {
	    return if (!$redfishSystem0);
	    redfishSystem0Response();
	    my $mainBoardModel = undef;
	    my $uuid = undef;
	    my $powerState = undef;
	    my $chassisModel = undef;
	    my $sysIPs = undef;
	    if ($redfishSystem0Response) {
		$mainBoardModel = $1 if ($redfishSystem0Response =~ m/\"Model\"\s*\:\s*\"(D[\d]+)\"/m);
		$uuid = redfish_simpleKeyString("UUID", $redfishSystem0Response);
		$powerState = redfish_simpleKeyString("PowerState", $redfishSystem0Response);
	    }
	    redfishManagersiRMCConfigurationSystemResponse();
	    if ($redfishManagersiRMCConfigurationSystemResponse) {
		$sysIPs = redfish_simpleKeyString("SystemIP", $redfishManagersiRMCConfigurationSystemResponse);
		$sysIPs =~ s/\\//g if ($sysIPs);
	    }
	    #
	    addStatusTopic("v",undef,"ServerInfo", undef);
	    addKeyValue("v","PowerState", $powerState);
	    addKeyLongValue("v","SystemIPs",$sysIPs);
	    addKeyValue("v","Uuid",$uuid);
	    addKeyValue("v","MainBoardModel",$mainBoardModel);
	    addMessage("v","\n");
	}
  } # redfishSystemInventoryInfo
  sub redfishSystemNotifyInformation {
	return if (!$hasiRMCS5Redfish or !$redfishSystem0);
	redfishSystem0Response();
	redfishChassis0Response();
	redfishManagersiRMCResponse();
	...
	my $osversion = undef;
	if ($redfishSystem0Response) {
	    $sysname	= redfish_simpleKeyString("HostName", $redfishSystem0Response);
	    $descr	= redfish_simpleKeyString("Description", $redfishSystem0Response);
	    my $out = $redfishSystem0Response;
	    $out =~ s/\n//gm;
	    $out =~ s/.*AssetTag//m;
	    $model	= redfish_simpleKeyString("Model", $out);
	}
	if ($redfishChassis0Response) {
	    $chassisModel = redfish_simpleKeyString("Type", $redfishChassis0Response);
	}
	if ($redfishManagersiRMCResponse) {
	    my $confLink = redfishExtractLink("iRMCConfiguration", $redfishManagersiRMCResponse);
	    my $dnsResponse = redfishGET($confLink . "/Dns");
	    $domain = redfish_simpleKeyString("Domain", $dnsResponse) if ($dnsResponse);
	}
	redfishManagersiRMCConfigurationSystemResponse();
	if ($redfishManagersiRMCConfigurationSystemResponse) {
	    my $systemResponse = $redfishManagersiRMCConfigurationSystemResponse;
	    $location	= redfish_simpleKeyString("Location", $systemResponse);
	    $contact	= redfish_simpleKeyString("Contact", $systemResponse);
	    $osdescr	= redfish_simpleKeyString("OperatingSystem", $systemResponse);
	    $osversion	= redfish_simpleKeyString("OperatingSystemVersion", $systemResponse);
	}
	#
	addName("n", $sysname);
	addKeyLongValue("n", "Domain", $domain);
	addKeyLongValue("n", "Description", $descr);
	addProductModel("n", undef, $model) if ($optServiceType !~ m/^R/i);
	addKeyLongValue("n", "Housing", $chassisModel);
	addLocationContact("n", $location, $contact);
	addKeyLongValue("n","OS",$osdescr);
	addKeyLongValue("n","OS-Revision",$osversion);
	    #addKeyValue("n","ParentMMB", $parent);
	    # PRIMEQUEST-MMB ?, MultiNode ?

	if ($optServiceType !~ m/^R/i) {
	    addExitCode(0) if ($optSystemInfo and $serverID);
	    if ($optSystemInfo and !$serverID) {
		addMessage("m"," - ATTENTION: Missing system information"); # never reached point
	    }
	}
  } # redfishSystemNotifyInformation
  sub redfishOverallStatusValues {
	return if (!$hasiRMCS5Redfish or !$redfishSystem0);
	redfishSystem0Response();
	#### Host Overall Status
	my $out = $redfishSystem0Response;
	$out =~ s/\n//gm;
	$out =~ s/.*ComputerSystem.Reset//m; # hack
	my $stateText = redfish_simpleKeyString("HealthRollup", $out);
	$statusOverall = redfish_code($stateText);
	if ($optChkSystem) {
	    addExitCode($statusOverall) if (defined $statusOverall);
	    addComponentStatus("m", "Overall", $stateText);
	}
	#### Environment
	if ($optChkEnvironment or $optChkEnv_Fan or $optChkEnv_Temp) {
	    redfish0ThermalResponse();
	    my $tempText    = redfish_simpleKeyString("TemperatureSumStatus", $redfish0ThermalResponse); 
	    my $fanText	    = redfish_simpleKeyString("FanSumStatus", $redfish0ThermalResponse);
	    $allTempStatus = redfish_code($tempText) if ($tempText);
	    $allFanStatus = redfish_code($fanText) if ($fanText);
	    $statusEnv = $allTempStatus;
	    $statusEnv = addTmpExitCode($allFanStatus, $statusEnv);
	    addExitCode($statusEnv) if ($optChkEnvironment and $optChkEnvironment != 999);
	    addComponentStatus("m", "Environment", $redfishCodeText{$statusEnv}) 
		if ($optChkEnvironment and defined $statusEnv);
	    addComponentStatus("m", "Fans", $fanText) if ($fanText and $optChkEnv_Fan);
	    addComponentStatus("m", "TemperatureSensors", $tempText) if ($tempText and $optChkEnv_Temp);
	} # environment
	#### Power & Voltage
	my $voltText = undef;
	if ($optChkPower or $optChkSystem or $optChkHardware or $optChkVoltage) {
	    redfish0PowerResponse();
	    my $psuText	    = redfish_simpleKeyString("PsuSumStatus", $redfish0PowerResponse);
	    $voltText	    = redfish_simpleKeyString("VoltageSumStatus", $redfish0PowerResponse);
	    $statusPower = redfish_code($psuText) if ($psuText);
	    $allVoltageStatus = redfish_code($voltText) if ($voltText);
	    addExitCode($statusPower) if ($optChkPower and $optChkPower != 999);
	    addComponentStatus("m", "PowerSupplies", $psuText) if ($optChkPower and $psuText);
	}
	#### SystemBoard
	if ($optChkSystem or $optChkHardware or $optChkCPU or $optChkMemMod or $optChkVoltage) {
	    redfishSystem0Response();
	    my $out = $redfishSystem0Response;
	    $out =~ s/\n//gm;
	    $out =~ s/.*ProcessorSummary/\"ProcessorSummary/m;
	    $out =~ s/\"MemorySummary.*$//m;
	    my $cpuText = redfish_simpleKeyString("Health", $out); 
	    $out = $redfishSystem0Response;
	    $out =~ s/\n//gm;
	    $out =~ s/.*MemorySummary/\"MemorySummary/m;
	    $out =~ s/\"Actions.*$//m;
	    my $memText = redfish_simpleKeyString("Health", $out);
	    $allCPUStatus = redfish_code($cpuText) if ($cpuText);
	    $allMemoryStatus = redfish_code($memText) if ($memText);
	    $statusSystemBoard = $allCPUStatus;
	    ...
	    addComponentStatus("m", "SystemBoard", $redfishCodeText{$statusSystemBoard}) 
		if (($optChkSystem or $optChkHardware) and defined $statusSystemBoard);
	    addComponentStatus("m", "CPU", $cpuText) if ($optChkCPU);
	    addComponentStatus("m", "MemoryModules", $memText) if ($optChkMemMod);
	    #$optChkHardware
	    addExitCode($statusSystemBoard) if ($optChkHardware and $optChkHardware != 999);
	}
	if ($optChkVoltage) {
	    addComponentStatus("m", "Voltages", $voltText);
	    addExitCode($allVoltageStatus) if ($optChkVoltage and $optChkVoltage != 999);
	}
	#### Storage
	if ($optChkStorage) {
	    my $out = redfishSysStoragesResponse();
	    my $count= 0;
	    $count = $1 if ($out =~ m/\"Members\@odata.count\"\s*\:\s*([\d]+)/m);
	    if ($count) {
		my $refLinks = redfishArrayLinks($out);
		my @links = ();
		if ($refLinks) {
		    @links = @{$refLinks};
		}
		foreach my $oneLink (@links) {
		    my $out = redfishGET($oneLink);
		    next if (!$out);
		    $out =~ s/\n//gm;
		    my $stateText = redfish_simpleKeyString("HealthRollup", $out);
		    my $tmpCode =undef;
		    $tmpCode = redfish_code($stateText) if ($stateText);
		    $statusMassStorage = addTmpExitCode($tmpCode, $statusMassStorage) if (defined $tmpCode);
		} # foreach
	    } else {
		$statusMassStorage = 3;
	    }
	    addComponentStatus("m", "MassStorage", $redfishCodeText{$statusMassStorage}) 
		if ($optChkStorage and defined $statusMassStorage);
	    addExitCode($statusMassStorage) if ($optChkStorage and $optChkStorage != 999);
	}
	#### DriverMonitor
	if ($optChkSystem or $optChkDrvMonitor) {
	    redfishSystem0Response();
	    my $drvText = redfish_simpleKeyString("DriverMonitorStatus", $redfishSystem0Response);
	    $statusDrvMonitor  = redfish_code($drvText) if ($drvText);
	    addComponentStatus("m", "DrvMonitor", $drvText) 
		if ((($optChkSystem and $optChkSystem==999) or $optChkDrvMonitor) and $drvText);
	    addExitCode($statusDrvMonitor) if ($optChkDrvMonitor and $optChkDrvMonitor != 999);
	}
  } # redfishOverallStatusValues
  ####
  sub redfishAllFanSensors {
	my $setExitCode = shift;
	my $notify = shift;
	my $verbose = shift;
	return if (!$hasiRMCS5Redfish);
	return if (!$notify and !$verbose and !$optChkFanPerformance);
	redfish0ThermalResponse();
	return if (!$redfish0ThermalResponse);
	my $count = undef;
	my $out = $redfish0ThermalResponse;
	$count = $1 if ($out =~ m/\"Fans\@odata.count\"\s*\:\s*([\d]+)/);
	return if (!$count);
	$out =~ s/\n//gm;
	$out =~ s/.*\"Fans\"/\"Fans\"/m;
	$out =~ s/\].*$/\]/m;
	#$out = "{ \"response\": { $out } }";
	$out =~ s/\"Fans\"\s*\://m;
	(my $rest, my $refArray) = jsonSplitArray($out);
	return if (!$refArray);
	my @array = @{$refArray};
	addTableHeader("v","Fans") if ($verbose and $#array > 0);
	foreach my $refobject (@array) {
	    next if (!$refobject);
	    next if (! $refobject->{"TYPE"} eq "OBJECT");
	    my $name = undef;	my $reading = undef;	my $readunits = undef;
	    my $statustext = undef; my $id = undef;
	    
	    my $refelem = jsonGetSubKeyValue($refobject,"Name");
	    $name = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"MemberId");
	    $id = $refelem->{"VALUE"} if ($refelem);
	    $refelem = jsonGetSubKeyValue($refobject,"Reading");
	    $reading = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"ReadingUnits");
	    $readunits = $refelem->{"VALUE"};
	    $readunits = "rpm" if ($readunits and $readunits eq "RPM");
	    my $refStatus = jsonGetSubKeyValue($refobject,"Status");
	    next if (!$refStatus or  $refStatus->{"TYPE"} ne "OBJECT");
		$refelem = jsonGetSubKeyValue($refStatus,"Health");
		$statustext = $refelem->{"VALUE"};
		$statustext = "NotPresent" if ($statustext and $statustext eq "null");
	    #
	    my $status = redfish_code($statustext);
	    my $medium = undef;
	    $medium = "v" if ($verbose);
	    $medium = "l" if (!$medium and $notify and defined $status
		and ($status == 1 or $status == 2 ));
	    if ($medium) {
		    addStatusTopic($medium,$statustext, "Fan", $id);
		    addName($medium,$name);
		    addKeyIntValue($medium,"Chassis", 0);
		    addKeyValueUnit($medium,"Speed", $reading, $readunits)
			if ($reading ne "null");
		    addMessage($medium,"\n");
	    }
	    if ($optChkFanPerformance and $reading ne "null") {
		    $name =~ s/[\s\,\.\$\(\)]+/_/g;
		    $name =~ s/_+/_/g;
		    addRpmToPerfdata($name, $reading, undef, undef);
	    }
	} # foreach
	# TODO other chassis
  } # redfishAllFanSensors
  sub redfishAllTemperatureSensors {
	my $setExitCode = shift;
	my $notify = shift;
	my $verbose = shift;
	return if (!$hasiRMCS5Redfish);
	#return if (!$notify and !$verbose);
	redfish0ThermalResponse();
	return if (!$redfish0ThermalResponse);
	my $count = undef;
	my $out = $redfish0ThermalResponse;
	$count = $1 if ($out =~ m/\"Temperatures\@odata.count\"\s*\:\s*([\d]+)/);
	return if (!$count);
	$out =~ s/\n//gm;
	$out =~ s/.*\"Temperatures\"//m;
	$out =~ s/^\s*\://m;
	(my $rest, my $refArray) = jsonSplitArray($out);
	return if (!$refArray);
	my @array = @{$refArray};
	addTableHeader("v","Temperature Sensors") if ($verbose and $#array > 0);
	foreach my $refobject (@array) {
	    next if (!$refobject);
	    next if (! $refobject->{"TYPE"} eq "OBJECT");
	    my $name = undef;	my $reading = undef;	 my $statustext = undef;
	    my $warn = undef;	my $crit = undef;	 my $id = undef;
	    my $refelem = jsonGetSubKeyValue($refobject,"Name");
	    $name = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"MemberId");
	    $id = $refelem->{"VALUE"} if ($refelem);
	    $refelem = jsonGetSubKeyValue($refobject,"ReadingCelsius");
	    $reading = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"UpperThresholdNonCritical");
	    $warn = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"UpperThresholdCritical");
	    $crit = $refelem->{"VALUE"};
	    my $refStatus = jsonGetSubKeyValue($refobject,"Status");
	    next if (!$refStatus or $refStatus->{"TYPE"} ne "OBJECT");
		$refelem = jsonGetSubKeyValue($refStatus,"Health");
	    ...
	    my $medium = undef;
	    $medium = "v" if ($verbose);
	    $medium = "l" if (!$medium and $notify and defined $status and ($status == 1 or $status == 2 ));
	    if ($medium) {
		    addStatusTopic($medium,$statustext, "Sensor", $id);
		    addName($medium,$name);
		    addCelsius($medium,$reading, $warn, $crit) if ($statustext ne "NotPresent");
		    addMessage($medium,"\n");
	    }
	    if ($statustext ne "NotPresent") { # performance
		    $name =~ s/[\s\,\.\$\(\)]+/_/g;
		    $name =~ s/_+/_/g;
		    addTemperatureToPerfdata($name, $reading, $warn, $crit)
				if (!$main::verboseTable);	    
	    }
	} # foreach
	# TODO other chassis
  } # redfishAllTemperatureSensors
  sub redfishStripNonJsonParts {
	my $out = shift;
	$out =~ s/\s*\"\@odata\.[^\n]*\n//gm;	# remove non-json-conform @odata lines
	$out =~ s/\s*\"\@Redfish\.[^\n]*\n//gm;	# remove non-json-conform @odata lines
	$out =~ s/\s*\"target[^\n]*\n//gm;	# remove too much escape signs
	$out =~ s/\"http\:[^\"]+/\"http/gm;	# remove too much escape signs
	$out =~ s/\[\s+\]/\[\]/gm;		# strip empty arrays
	$out =~ s/\{\s+\}/\{\}/gm;		# strip empty objects
	$out =~ s/\:null/\:\"null\"/gm;		# replace Redfish-null to String
	$out =~ s/\[\]/\"null\"/gm;		# mark empty arrays
  ...
  sub redfishAllPowerSupplies {
	...
	return if (!$redfish0PowerResponse);
	my $count = undef;
	my $out = $redfish0PowerResponse;
	$count = $1 if ($out =~ m/\"PowerSupplies\@odata.count\"\s*\:\s*([\d]+)/);
	return if (!$count);
	# Check data in Oem part
	my $skipConsumption = 0;
	$skipConsumption = 1 if ($out =~ m/\"ChassisPowerSensors\"\s*\:\s*null/m);
	my %psuCurrent = ();
	$out = redfishStripNonJsonParts($out);
	$skipConsumption = 1 if ($out !~ m/\"ChassisPowerSensors\"/m);
	$skipConsumption = 1 if ($out =~ m/\"ChassisPowerSensors\"\s*\:\s*\"null\"/m);
	if ($skipConsumption == 0) {
	    $out =~ s/\n//gm;
	    $out =~ s/.*\"ChassisPowerSensors\"//m;
	    $out =~ s/^\s*\://m;
	    (my $rest, my $refArray) = jsonSplitArray($out);
	    return if (!$refArray);
	    my @array = @{$refArray};
		...
		next if (! $refobject->{"TYPE"} eq "OBJECT");
		my $entityId = undef;
		my $refelem = jsonGetSubKeyValue($refobject,"EntityId");
		$entityId = $refelem->{"VALUE"};
		next if (!$entityId or $entityId ne "Power Supply");
		my $designation = undef;
		my $currentPowerConsumptionW = undef;
		$refelem = jsonGetSubKeyValue($refobject,"Designation");
		$designation = $refelem->{"VALUE"};
		$refelem = jsonGetSubKeyValue($refobject,"CurrentPowerConsumptionW");
		$currentPowerConsumptionW = $refelem->{"VALUE"};
		$psuCurrent{$designation} = $currentPowerConsumptionW 
		    if (defined $currentPowerConsumptionW and $currentPowerConsumptionW ne "null");
	    } #foreach
	} #get consumption values
	# Check rest
	$out =~ s/.*\"PowerSupplies\"//m;
	$out =~ s/^\s*\://m;

	(my $rest, my $refArray) = jsonSplitArray($out);
	return if (!$refArray);
	my @array = @{$refArray};
	addTableHeader("v","Power Supplies") if ($verbose and $#array > 0);
	foreach my $refobject (@array) {
	    next if (!$refobject);
	    next if (! $refobject->{"TYPE"} eq "OBJECT");
	    my $name = undef;	my $reading = undef;	 my $statustext = undef;
	    my $powerCapacityWatts = undef;		 my $id = undef;
	    my $refelem = jsonGetSubKeyValue($refobject,"Name");
	    $name = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"MemberId");
	    $id = $refelem->{"VALUE"} if ($refelem);
	    $refelem = jsonGetSubKeyValue($refobject,"PowerCapacityWatts");
	    $powerCapacityWatts = $refelem->{"VALUE"};
	    $powerCapacityWatts = undef if (defined $powerCapacityWatts and $powerCapacityWatts eq "null");
	    my $refStatus = jsonGetSubKeyValue($refobject,"Status");
	    next if (!$refStatus or $refStatus->{"TYPE"} ne "OBJECT");
		$refelem = jsonGetSubKeyValue($refStatus,"Health");
		$statustext = $refelem->{"VALUE"};
		$statustext = "NotPresent" if ($statustext and $statustext eq "null");
	    #
	    my $load = $psuCurrent{$name};
	    my $status = redfish_code($statustext);
	    my $medium = undef;
	    $medium = "v" if ($verbose);
	    $medium = "l" if (!$medium and $notify and defined $status and ($status == 1 or $status == 2 ));
	    if ($medium) {
		    addStatusTopic($medium,$statustext, "PSU", $id);
		    addName($medium,$name);
		    addKeyWatt($medium, "CurrentLoad", $load);
		    addKeyWatt($medium, "Capacity", $powerCapacityWatts);
		    addMessage($medium,"\n");
	    }
	} #foreach
	# TODO other chassis
  } # redfishAllPowerSupplies
  sub redfishAllPowerConsumption {
	my $setExitCode = shift;
	my $notify = shift;
	my $verbose = shift;
	return if (!$hasiRMCS5Redfish);
	redfish0PowerResponse();
	return if (!$redfish0PowerResponse);
	# ChassisPowerConsumption
	my $out = $redfish0PowerResponse;
	$out =~ s/\n//gm;
	$out =~ s/.*\"ChassisPowerConsumption\"//m;
	$out =~ s/^\s*\://m;
	$out =~ s/ChassisPowerSensors.*$//m;
	
	my $current	= redfish_simpleKeyNumber("CurrentPowerConsumptionW", $out);	
	my $min		= redfish_simpleKeyNumber("MinimumPowerW", $out);    
	my $peak	= redfish_simpleKeyNumber("PeakPowerW", $out);
	my $average	= redfish_simpleKeyNumber("AveragePowerW", $out);	
	my $max		= redfish_simpleKeyNumber("CurrentMaximumPowerW", $out);
	my $warn	= redfish_simpleKeyNumber("WarningThresholdW", $out); # newer versions
	my $crit	= redfish_simpleKeyNumber("CriticalThresholdW", $out); # newer versions
	#
	my $medium = undef;
	$medium = "v" if ($verbose);
	#$medium = "l" if ($notify and $status);
	if ($medium and defined $current) {
		addTableHeader("v","Power Consumption") if ($verbose and defined $current);
		addStatusTopic($medium,undef, "PSCons", "");
		addName($medium,"Total Power");
		addKeyWatt($medium, "Current", $current);
		addKeyWatt($medium, "Average", $average);
		addKeyWatt($medium, "Peak", $peak);
		addKeyWatt($medium, "Minimum", $min);
		addKeyWatt($medium, "NominalMax", $max);
		addMessage($medium,"\n");
	}
	addPowerConsumptionToPerfdata($current,$warn,$crit)		if (!$main::verboseTable);
	$average = 0 if (!defined $average);
	addKeyUnitToPerfdata("PowerConsumptionAverage","Watt", $average) if (!$main::verboseTable);
	# TODO other chassis
	# PowerControl
	$out = $redfish0PowerResponse;
	my $count = undef;
	$count = $1 if ($out =~ m/\"PowerControl\@odata.count\"\s*\:\s*([\d]+)/);
	return if (!$count);
	$out =~ s/\n//gm;
	$out =~ s/.*\"PowerControl\"//m;
	$out =~ s/^\s*\://m;
	(my $rest, my $refArray) = jsonSplitArray($out);
	return if (!$refArray);
	my @array = @{$refArray};
	addTableHeader("v","Power Control Adapter") if ($verbose and defined $current);
	foreach my $refobject (@array) {
	    next if (!$refobject);
	    next if (! $refobject->{"TYPE"} eq "OBJECT");
	    my $name = undef;	my $statustext = undef;	    my $addtext = undef;
	    my $memberid = undef;
	    my $refelem = jsonGetSubKeyValue($refobject,"Name");
	    $name = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"MemberId");
	    $memberid = $refelem->{"VALUE"};
	    my $refStatus = jsonGetSubKeyValue($refobject,"Status");
	    next if (!$refStatus or $refStatus->{"TYPE"} ne "OBJECT");
		$refelem = jsonGetSubKeyValue($refStatus,"Health");
		$statustext = $refelem->{"VALUE"};
		$statustext = "NotPresent" if ($statustext and $statustext eq "null");
	    my $refOem = jsonGetSubKeyValue($refobject,"Oem");
	    next if (!$refOem or $refOem->{"TYPE"} ne "OBJECT");
		my $fts = jsonGetSubKeyValue($refOem,"ts_fujitsu");
		next if (!$fts or $fts->{"TYPE"} ne "OBJECT");
		    $refelem = jsonGetSubKeyValue($fts,"LegacyStatus");
		    $addtext = $refelem->{"VALUE"};
	    #
	    my $status = redfish_code($statustext);
	    my $medium = undef;
	    $medium = "v" if ($verbose);
	    $medium = "l" if (!$medium and $notify and $status);
	    if ($medium and defined $current) {
		    addStatusTopic($medium,$statustext, "PowerControl", $memberid);
		    addName($medium,$name);
		    addKeyLongValue($medium, "LegacyStatus", $addtext);
		    addMessage($medium,"\n");
	    }
	} # foreach
	# TODO other chassis
  } # redfishAllPowerConsumption
  sub redfishAllCPU {
	my $setExitCode = shift;
	my $notify = shift;
	my $verbose = shift;
	...
	redfishSystem0Response();
	return if (!$redfishSystem0Response);
	my $cpuLink = redfishExtractLink("Processors", $redfishSystem0Response);
	return if (!$cpuLink);
	my $cpuStream = redfishGET($cpuLink);
	my $refArray = redfishArrayLinks($cpuStream);
	return if (!$refArray);
	my @array = @{$refArray};
	return if ($#array < 0);
	addTableHeader("v","CPU Table") if ($verbose);
	foreach my $oneCpuLink (@array) {
	    next if (!$oneCpuLink);
	    my $oneCpuStream = redfishGET($oneCpuLink);
	    next if (!$oneCpuStream);
	    my $out = $oneCpuStream;
	    my $name = redfish_simpleKeyString("Name", $out);	
	    my $id = redfish_simpleKeyString("Id", $out);	
	    my $statustext = redfish_simpleKeyString("Health", $out);	    
	    my $InstructionSet = redfish_simpleKeyString("InstructionSet", $out); 
	    my $speed = redfish_simpleKeyNumber("MaxSpeedMHz", $out);	
	    my $manufact = redfish_simpleKeyString("Manufacturer", $out);	    
	    my $model = redfish_simpleKeyString("Model", $out);;
	    $model = undef if ($model and $model eq "");
	    my $status = redfish_code($statustext);
	    my $medium = undef;		   
	   
	    $medium = "v" if ($verbose);
	    $medium = "l" if (!$medium and $notify and defined $status and $status >=1 and $status <= 2);
	    if ($medium) {
		    addStatusTopic($medium,$statustext, "CPU", $id);
		    addName($medium,$name);
		    addKeyMHz($medium, "Speed", $speed);
		    addKeyLongValue($medium,"InstructionSet", $InstructionSet);
		    addProductModel($medium,undef,$model);
		    addKeyLongValue($medium,"Manufacturer", $manufact);
		    addMessage($medium,"\n");
	    } 
  ...
  sub redfishAllVoltages {
	...
	my $out = $redfish0PowerResponse;
	my $count = undef;
	$count = $1 if ($out =~ m/\"Voltages\@odata.count\"\s*\:\s*([\d]+)/);
	return if (!$count);
	$out =~ s/\n//gm;
	$out =~ s/.*\"Voltages\"//m;
	$out =~ s/^\s*\://m;
	...
	return if (!$refArray);
	my @array = @{$refArray};
	addTableHeader("v","Voltages") if ($verbose and $#array > 0);
	foreach my $refobject (@array) {
	    next if (!$refobject);
	    next if (! $refobject->{"TYPE"} eq "OBJECT");
	    my $name = undef;	my $reading = undef;	 my $statustext = undef;
	    my $lowercrit = undef;  my $uppercrit = undef; my $id = undef;
	    my $refelem = jsonGetSubKeyValue($refobject,"Name");
	    $name = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"MemberId");
	    $id = $refelem->{"VALUE"} if ($refelem);
	    $refelem = jsonGetSubKeyValue($refobject,"ReadingVolts");
	    $reading = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"LowerThresholdCritical");
	    $lowercrit = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"UpperThresholdCritical");
	    $uppercrit = $refelem->{"VALUE"};
	    $reading =~ s/0+$// if ($reading);
	    $lowercrit =~ s/0+$// if ($lowercrit);
	    $uppercrit =~ s/0+$// if ($uppercrit);
	    my $refStatus = jsonGetSubKeyValue($refobject,"Status");
	    next if (!$refStatus or $refStatus->{"TYPE"} ne "OBJECT");
		$refelem = jsonGetSubKeyValue($refStatus,"Health");
		$statustext = $refelem->{"VALUE"};
	    ...
	    $medium = "l" if (!$medium and $notify and defined $status and ($status == 1 or $status == 2 ));
	    if ($medium) {
		    addStatusTopic($medium,$statustext, "Voltage", $id);
		    addName($medium,$name);
		    addKeyIntValueUnit($medium,"Current",$reading,"V");
		    addKeyIntValueUnit($medium,"LowerThresholdCritical",$lowercrit,"V");
		    addKeyIntValueUnit($medium,"UpperThresholdCritical",$uppercrit,"V");
		    addMessage($medium,"\n");
	    }
	} # foreach
  } # redfishAllVoltages
  sub redfishAllMemoryModules {
	my $setExitCode = shift;
	my $notify = shift;
	my $verbose = shift;
	return if (!$hasiRMCS5Redfish);
	...
	redfishSystem0Response();
	return if (!$redfishSystem0Response);
	my $memmodLink = redfishExtractLink("Memory", $redfishSystem0Response);
	return if (!$memmodLink);
	if ($useRedfishExpand) {
	    redfishAllMemoryModulesExpanded($notify,$verbose,$memmodLink);
	    return;
	}
	my $memStream = redfishGET($memmodLink);
	my $refArray = redfishArrayLinks($memStream);
	return if (!$refArray);
	my @array = @{$refArray};
	return if ($#array < 0);
	addTableHeader("v","Memory Modules Table") if ($verbose);
	foreach my $oneLink (@array) {
	    next if (!$oneLink);
	    my $oneStream = redfishGET($oneLink);
	    next if (!$oneStream);
	    my $id = redfish_simpleKeyString("Id", $oneStream);
	    my $name = redfish_simpleKeyString("DeviceLocator", $oneStream);
	    my $voltage = redfish_simpleKeyString("Voltage", $oneStream);
	    my $type = redfish_simpleKeyString("MemoryDeviceType", $oneStream);
	    my $maxfreq = redfish_simpleKeyNumber("MaxFrequencyMHz", $oneStream);
	    my $capacity = redfish_simpleKeyNumber("CapacityMiB", $oneStream);
	    my $manufact = redfish_simpleKeyString("Manufacturer", $oneStream);
	    my $ser = redfish_simpleKeyString("SerialNumber", $oneStream);
	    my $speed = redfish_simpleKeyNumber("OperatingSpeedMhz", $oneStream);
	    my $statustext = redfish_simpleKeyString("Health", $oneStream);
	    my $configstate  = redfish_simpleKeyString("ConfigurationState", $oneStream);
	    next if (!$type);
	    my $status = redfish_code($statustext);
	    my $medium = undef;		   
	    $medium = "v" if ($verbose);
	    $medium = "l" if (!$medium and $notify and defined $status and $status >=1 and $status <= 2);
	    if ($medium) {
		    addStatusTopic($medium,$statustext, "Memory", $id);
		    addName($medium,$name);
		    addKeyLongValue($medium,"Type", $type);
		    addKeyValueUnit($medium,"Capacity", $capacity, "MiB");
		    addKeyMHz($medium, "Speed", $speed);
		    addKeyMHz($medium, "MaxFrequency", $maxfreq);
		    addKeyValue($medium, "Voltage", $voltage);
		    addKeyLongValue($medium,"Manufacturer", $manufact);
		    addSerialIDs($medium, $ser);
		    addKeyLongValue($medium,"Configuration", $configstate);
		    addMessage($medium,"\n");
	    } 
	} # foreach
  } # redfishAllMemoryModules
  sub redfishAllMemoryModulesExpanded {
	my $notify = shift;
	my $verbose = shift;
	my $memmodLink = shift;
	my $out = redfishGET($memmodLink . '?$expand=Members');
	#
	$out =~ s/\s*\"\@odata\.[^\n]*\n//gm;	# remove non-json-conform @odata lines
	$out =~ s/\s*\"\@Redfish\.[^\n]*\n//gm;	# remove non-json-conform @odata lines
	$out =~ s/\s*\"target[^\n]*\n//gm;	# remove too much escape signs
	...
	$out =~ s/.*\"Members\"//m;
	$out =~ s/^\s*\://m;
	$out =~ s/\"Members\@odata\.count\".*//m;
	$out =~ s/\@/_/gm;
	#
	(my $rest, my $refArray) = jsonSplitArray($out);
	return if (!$refArray);
	my @array = @{$refArray};
	addTableHeader("v","Memory Modules Table") if ($verbose and $#array > 0);
	foreach my $refobject (@array) {
	    next if (!$refobject);
	    next if (! $refobject->{"TYPE"} eq "OBJECT");
	    my $refelem = jsonGetSubKeyValue($refobject,"Id");
	    my $id = undef; $id = $refelem->{"VALUE"} if ($refelem);
	    $refelem = jsonGetSubKeyValue($refobject,"DeviceLocator");
	    my $name = undef; $name = $refelem->{"VALUE"} if ($refelem);
	    $refelem = jsonGetSubKeyValue($refobject,"MemoryDeviceType");
	    my $type = undef; $type = $refelem->{"VALUE"} if ($refelem);
	    next if (!$type);
	    $refelem = jsonGetSubKeyValue($refobject,"CapacityMiB");
	    my $capacity = undef; $capacity = $refelem->{"VALUE"} if ($refelem);
	    $refelem = jsonGetSubKeyValue($refobject,"Manufacturer");
	    my $manufact = undef; $manufact = $refelem->{"VALUE"} if ($refelem);
	    $refelem = jsonGetSubKeyValue($refobject,"SerialNumber");
	    my $ser = undef; $ser = $refelem->{"VALUE"} if ($refelem);
	    $refelem = jsonGetSubKeyValue($refobject,"OperatingSpeedMhz");
	    my $speed = undef; $speed = $refelem->{"VALUE"} if ($refelem);
	    my $refStatus = jsonGetSubKeyValue($refobject,"Status");
	    my $statustext = undef;
	    next if (!$refStatus or $refStatus->{"TYPE"} ne "OBJECT");
		$refelem = jsonGetSubKeyValue($refStatus,"Health");
	    ...
	    #
	    my $refOem = jsonGetSubKeyValue($refobject,"Oem");
	    my $voltage = undef; my $maxfreq = undef; my $configstate = undef;
	    next if (!$refOem or $refOem->{"TYPE"} ne "OBJECT");
		my $fts = jsonGetSubKeyValue($refOem,"ts_fujitsu");
		next if (!$fts or $fts->{"TYPE"} ne "OBJECT");
		    $refelem = jsonGetSubKeyValue($fts,"Voltage");
		    $voltage = $refelem->{"VALUE"} if ($refelem);
		    $refelem = jsonGetSubKeyValue($fts,"MaxFrequencyMHz");
		    $maxfreq = $refelem->{"VALUE"} if ($refelem);
		    $refelem = jsonGetSubKeyValue($fts,"ConfigurationState");
		    $configstate  = $refelem->{"VALUE"} if ($refelem);
	    #
	    $capacity = undef	if ($capacity !~ m/\d+/);
	    $speed = undef	if ($speed !~ m/\d+/);
	    $maxfreq = undef	if ($maxfreq !~ m/\d+/);
	    $voltage = undef	if ($voltage !~ m/\d+/);
	    $manufact = undef	if ($manufact eq "null");
	    $type = undef	if ($type eq "null");
	    $ser = undef	if ($ser eq "null");
	    my $status = redfish_code($statustext);
	    my $medium = undef;		   
	    $medium = "v" if ($verbose);
	    $medium = "l" if (!$medium and $notify and defined $status and $status >=1 and $status <= 2);
  ...
  } # redfishAllMemoryModulesExpanded
  sub redfishAllDrvMonAdapter {
	my $setExitCode = shift;
	my $notify = shift;
	my $verbose = shift;
	return if (!$hasiRMCS5Redfish);
	return if (!$notify and !$verbose);
	return if (!defined $statusDrvMonitor or $statusDrvMonitor == 3);
	redfishSystem0Response();
	return if (!$redfishSystem0Response);
	my $out = $redfishSystem0Response;
	$out =~ s/\n//gm;
	$out =~ s/.*\"MonitoredComponents\"//m;
	$out =~ s/^\s*\://m;
	(my $rest, my $refArray) = jsonSplitArray($out);
	return if (!$refArray);
	my @array = @{$refArray};
	addTableHeader("v","MonitoredComponents") if ($verbose and $#array > 0);
	foreach my $refobject (@array) {
	    next if (!$refobject);
	    next if (! $refobject->{"TYPE"} eq "OBJECT");
	    my $name = undef;	my $location = undef;	 my $statustext = undef;
	    my $refelem = jsonGetSubKeyValue($refobject,"Designation");
	    $name = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"Location");
	    $location = $refelem->{"VALUE"};
	    $refelem = jsonGetSubKeyValue($refobject,"Status");
	    $statustext = $refelem->{"VALUE"};
	    $statustext = "NotPresent" if ($statustext and $statustext eq "null");
	    my $status = redfish_code($statustext);
	    my $medium = undef;
	    ...
	    $medium = "l" if (!$medium and $notify and defined $status and ($status == 1 or $status == 2 ));
	    if ($medium) {
		    addStatusTopic($medium,$statustext, "Component", "");
		    addName($medium,$name);
		    addKeyLongValue($medium,"Location", $location);
		    addMessage($medium,"\n");
	    }
	} # foreach
  } # redfishAllDrvMonAdapter
  sub redfishAllStorageAdapter {
	my $setExitCode = shift;
	my $notify = shift;
	my $verbose = shift;
	return if (!$hasiRMCS5Redfish);
	return if (!$notify and !$verbose);
	return if ($statusMassStorage and $statusMassStorage == 3);
	#
	my $out = redfishSysStoragesResponse();
	my $count= 0;
	$count = $1 if ($out =~ m/\"Members\@odata.count\"\s*\:\s*([\d]+)/m);
	...
	my $refLinks = redfishArrayLinks($out);
	my @links = ();
	   @links = @{$refLinks} if ($refLinks);
	my @adapterResponse = ();
	foreach my $oneLink (@links) {
	    my $out = redfishGET($oneLink);
	    next if (!$out);
	    push(@adapterResponse, $out);
	} # foreach link
	addTableHeader("v","RAID Adapter") if ($verbose);
	foreach my $out (@adapterResponse) {
	    next if (!$out);
	    $out =~ s/\n//gm;
	    my $name = redfish_simpleKeyString("Name", $out);
	    my $stateText = redfish_simpleKeyString("HealthRollup", $out);
	    my $status = redfish_code($stateText);
	    my $medium = undef;
	    $medium = "v" if ($verbose);
	    $medium = "l" if ($notify and $status > 0 and $status < 3);
	    if ($medium) {
		    addStatusTopic($medium,$stateText, "RAIDAdapter", "");
		    addName($medium,$name);
		    addMessage($medium,"\n");
	    }
	} # foreach storage
	...
	foreach my $out (@adapterResponse) {
	    next if (!$out);
	    $out =~ s/\n//gm;
	    my $adapter = redfish_simpleKeyString("Name", $out);
	    my $count = 0;
	    $count = $1 if ($out =~ m/\"StorageControllers\@odata.count\"\:\s*(\d+)/m);
	    next if ($count < 1);
	    addTableHeader("v","RAID Controller") if (!$printHeader);
	    $printHeader = 1;
	    $out =~ s/.*\"StorageControllers\"//m;
	    $out =~ s/\s*\://;
	    (my $rest, my $refArray) = jsonSplitArray($out);
	    return if (!$refArray);
	    my @array = @{$refArray};
		...
		next if (!$refobject);
		next if (! $refobject->{"TYPE"} eq "OBJECT");
		my $refelem = jsonGetSubKeyValue($refobject,"SpeedGbps");
		my $speed = $refelem->{"VALUE"} if ($refelem);
		$refelem = jsonGetSubKeyValue($refobject,"FirmwareVersion");
		my $fwversion = $refelem->{"VALUE"} if ($refelem);
		$refelem = jsonGetSubKeyValue($refobject,"Manufacturer");
		my $manu = $refelem->{"VALUE"} if ($refelem);
		$refelem = jsonGetSubKeyValue($refobject,"Model");
		my $model = $refelem->{"VALUE"} if ($refelem);
		$refelem = jsonGetSubKeyValue($refobject,"SerialNumber");
		my $sid = $refelem->{"VALUE"} if ($refelem);
		$sid =~ s/^0+//;
		my $refStatus = jsonGetSubKeyValue($refobject,"Status");
		next if (!$refStatus or $refStatus->{"TYPE"} ne "OBJECT");
		    $refelem = jsonGetSubKeyValue($refStatus,"HealthRollup");
		    my $statustext = $refelem->{"VALUE"} if ($refelem);
		    $statustext = "NotPresent" if ($statustext and $statustext eq "null");
		my $status = redfish_code($statustext);
		my $refOem = jsonGetSubKeyValue($refobject,"Oem");
		next if (!$refOem or $refOem->{"TYPE"} ne "OBJECT");
		    my $fts = jsonGetSubKeyValue($refOem,"ts_fujitsu");
		    next if (!$fts or $fts->{"TYPE"} ne "OBJECT");
			$refelem = jsonGetSubKeyValue($fts,"VendorId");
			my $pciVendorID = $refelem->{"VALUE"} if ($refelem);
			$refelem = jsonGetSubKeyValue($fts,"VendorId");
			my $pciDeviceID = $refelem->{"VALUE"} if ($refelem);
			$refelem = jsonGetSubKeyValue($fts,"SubsystemVendorId");
			my $pciSubVendorID = $refelem->{"VALUE"} if ($refelem);
			$refelem = jsonGetSubKeyValue($fts,"SubsystemId");
			my $pciSubDeviceID = $refelem->{"VALUE"} if ($refelem);
			my $pcivendor = "$pciVendorID,$pciSubVendorID,$pciDeviceID,$pciSubDeviceID";
			#
			$refelem = jsonGetSubKeyValue($fts,"FirmwarePackageVersion");
			my $package = $refelem->{"VALUE"} if ($refelem);
			$refelem = jsonGetSubKeyValue($fts,"BiosVersion");
			my $bios = $refelem->{"VALUE"} if ($refelem);
			#statustext
		...
		$medium = "l" if ($notify and $status > 0 and $status < 3);
		if ($medium) {
		        addStatusTopic($medium,$statustext, "RAIDController", "");
			addKeyValueUnit($medium,"Speed", $speed, "Gbps");
		        addKeyLongValue($medium,"Adapter",$adapter);
			addProductModel($medium,undef, $model);
			addSerialIDs($medium,$sid);
			addKeyValue($medium,"Version", $fwversion);
			addKeyValue($medium,"Bios", $bios);
			addKeyValue($medium,"Package", $package);
			addKeyLongValue($medium,"PCI-VendorInformation", $pcivendor);
			addKeyLongValue($medium,"Manufacturer", $manu);
			#addKeyValue($medium,"PCI-Bus-Dev-Fct",$pciconcat);
		        addMessage($medium,"\n");
		}
	    } # foreach controller	    
	} # foreach storage
	...
	foreach my $out (@adapterResponse) {
	    ...
	    my $adapter = redfish_simpleKeyString("Name", $out);
	    my $count = 0;
	    $count = $1 if ($out =~ m/\"Drives\@odata.count\"\:\s*(\d+)/m);
	    next if ($count < 1);
	    addTableHeader("v","RAID Drives") if (!$printHeader);
	    $printHeader = 1;
	    redfishStorageDrives($notify,$verbose,$adapter,$out);
	} # foreach storage
	#### RAID Volume
	$printHeader = 0;
	foreach my $out (@adapterResponse) {
	    ...
	    $out =~ s/\n//gm;
	    my $adapter = redfish_simpleKeyString("Name", $out);
	    my $volumesLink = redfishExtractLink("Volumes", $out);
	    next if (!$volumesLink);
	    my $volumesResponse = redfishGET($volumesLink);
	    next if (!$volumesResponse);
	    addTableHeader("v","RAID Volumes") if (!$printHeader);
	    $printHeader = 1;
	    redfishStorageVolumes($notify, $verbose, $adapter, $volumesResponse);
	} # foreach storage
  } # redfishAllStorageAdapter
  ####
  sub redfishStorageVolumes {
	my $notify = shift;
	my $verbose = shift;
	my $adapter = shift;
	my $volumesResponse = shift;
	my $refArray = redfishArrayLinks($volumesResponse);
	return if (!$refArray);
	my @array = @{$refArray};
	return if ($#array < 0);
	foreach my $link (@array) {
	    next if (!$link);
	    my $response = redfishGET($link);
	    next if (!$response);
	    my $id	    = redfish_simpleKeyNumber("VolumeNumber", $response);
	    my $name	    = redfish_simpleKeyString("Name", $response);
	    my $statustext  = redfish_simpleKeyString("Health", $response);
	    my $raidlevel   = redfish_simpleKeyString("RaidLevel", $response);
	    my $sizeB	    = redfish_simpleKeyNumber("SizeBytes", $response);
	    my $capacityB    = redfish_simpleKeyNumber("CapacityBytes", $response);
	    my $count = 0;
	    $count = $1 if ($response =~ m/\"Drives\@odata.count\"\:\s*(\d+)/m);
	    my $driveAssign = undef;
	    if ($count > 0) {
		my $out = $response;
		$out =~ s/\n//gm;
		$out =~ s/.*\"Drives\"//m;
		...
		my @driveLinks = ();
		while ($out) {
		    my $oneLink = redfish_simpleKeyString("odata.id", $out);
		    $oneLink =~ s/\\//g if ($oneLink);
		    push (@driveLinks, $oneLink) if ($oneLink);
		    if ($out =~ m/\,/) {
			$out =~ s/^[^\,]*\,//m;
		    } else  {
			$out = undef;
		...
		foreach my $oneLink (@driveLinks) {
		    next if (!$oneLink);
		    my $nr = undef;
		    $nr = $1 if ($oneLink =~ m/(\d+)$/);
		    next if (!defined $nr);
		    $driveAssign .= "," if (defined $driveAssign);
		    $driveAssign .= "$nr";
		} # foreach
	    }

	    $sizeB =~ s/\.0+$// if ($sizeB);
	    $capacityB =~ s/\.0+$// if ($capacityB);
	    my $capacity = undef;   my $size = undef;
	    if ($sizeB >= 1024*1024) {
		$size = ($sizeB - $sizeB%1024) / 1024; # for KB
		$size = ($size - $size%1024) / 1024 if ($size); # for MB
	    }
	    if ($capacityB >= 1024+1024) {
		$capacity = ($capacityB - $capacityB%1024) / 1024; # for KB
	    ...
	    $medium = "l" if ($notify and $status > 0 and $status < 3);
	    if ($medium) {
		    addStatusTopic($medium,$statustext, "RAIDVolume", $id);
		    addName($medium,$name);
		    addKeyLongValue($medium,"Adapter",$adapter);
		    addKeyValue($medium,"Level", $raidlevel);
		    addKeyValueUnit($medium,"Size", $sizeB, "B") if (!$size);
		    addKeyValueUnit($medium,"Capacity", $capacityB, "B") if (!$capacity);
		    addKeyMB($medium,"Size",$size) if ($size);
		    addKeyMB($medium,"Capacity",$capacity) if ($capacity);
		    addKeyLongValue($medium,"AsssignedDrives", $driveAssign);
		    addMessage($medium,"\n");
	    }
	} # foreach
  } # redfishStorageVolumes
  sub redfishStorageDrives {
	my $notify = shift;
	my $verbose = shift;
	my $adapter = shift;
	my $out = shift;
	...
	foreach my $oneLink (@driveLinks) {
	    next if (!$oneLink);
	    my $response = redfishGET($oneLink);
	    next if (!$response);
	    my $slot	= redfish_simpleKeyNumber("SlotNumber", $response);
	    my $device	= redfish_simpleKeyNumber("DeviceNumber", $response);
	    my $powerst	= redfish_simpleKeyString("PowerStatus", $response);
	    my $temp	= redfish_simpleKeyNumber("TemperatureC", $response);
	    my $detail	= redfish_simpleKeyString("StatusDetails", $response);
	    my $id	= redfish_simpleKeyString("Id", $response);
	    my $name	= redfish_simpleKeyString("Name", $response);
	    my $model	= redfish_simpleKeyString("Model", $response);
	    my $statustext = redfish_simpleKeyString("Health", $response);
	    my $capacityB = redfish_simpleKeyNumber("CapacityBytes", $response);
	    my $protocol = redfish_simpleKeyString("Protocol", $response);
	    my $manu	= redfish_simpleKeyString("Manufacturer", $response);
	    my $sid	= redfish_simpleKeyString("SerialNumber", $response);
	    my $lifetime= redfish_simpleKeyNumber("PredictedMediaLifeLeftPercent", $response);
	    my $type	= redfish_simpleKeyString("MediaType", $response);
	    $capacityB =~ s/\.0+$// if ($capacityB);
	    my $capacity = undef;   my $size = undef;
	    if ($capacityB >= 1024+1024) {
		$capacity = ($capacityB - $capacityB%1024) / 1024; # for KB
	    ...
	    $medium = "l" if ($notify and $status > 0 and $status < 3);
	    if ($medium) {
		    addStatusTopic($medium,$statustext, "RAIDDrive", $id);
		    addName($medium,$name);
		    addKeyLongValue($medium,"Adapter",$adapter);
		    addProductModel($medium,undef,$model);
		    addKeyValue($medium,"Protocol",$protocol);
		    addKeyValue($medium,"MediaType",$type);
		    addKeyIntValue($medium,"Slot", $slot) if (defined $slot && $slot=~m/\d+/);
		    addKeyIntValue($medium,"DeviceNr", $device);
		    addKeyValueUnit($medium,"Capacity", $capacityB, "B") if (!$capacity);
		    addKeyMB($medium,"Capacity",$capacity) if ($capacity);
		    addCelsius($medium, $temp);
		    addKeyLongValue($medium,"PowerStatus", $powerst);
		    addKeyLongValue($medium,"Manufacturer", $manu);
		    addSerialIDs($medium,$sid);
		    addKeyLongValue($medium,"StatusDetail", $detail);
		    addKeyValueUnit($medium,"PredictedMediaLifetimeLeft", $lifetime, "%");
		    addMessage($medium,"\n");
	    }
	} # foreach
  } # redfishStorageDrives
#########################################################################
# ANY REST Provider 
#########################################################################
  sub getAllFanSensors {
	...
	return redfishAllFanSensors($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^ired/i);
  } # getAllFanSensors
  sub getAllTemperatureSensors {
	...
	return iRMCReportAllTemperatureSensors($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^R/i);
	return redfishAllTemperatureSensors($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^ired/i);
  ...
  sub getAllPowerSupplies {
	...
	return iRMCReportAllPowerSupplies($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^R/i);
	return redfishAllPowerSupplies($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^ired/i);
  } # getAllPowerSupplies
  sub getAllPowerConsumption {
	...
	return iRMCReportAllPowerConsumption($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^R/i);
	return redfishAllPowerConsumption($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^ired/i);
  ...
  sub getAllCPU {
	...
	return iRMCReportAllCPU($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^R/i);
	return redfishAllCPU($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^ired/i);
  } # getAllCPU
  sub getAllVoltages {
 	my $setExitCode = shift;
	...
	return iRMCReportAllVoltages($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^R/i);
	return redfishAllVoltages($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^ired/i);
  } # getAllVoltages
  sub getAllMemoryModules {
	...
	return iRMCReportAllMemoryModules($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^R/i);
	return redfishAllMemoryModules($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^ired/i);
  } # getAllMemoryModules
  sub getAllDrvMonAdapter {
	...
	return agentAllDrvMonAdapter($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^A/i);
	return redfishAllDrvMonAdapter($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^ired/i);
  } # getAllDrvMonAdapter
...
=end NOIRMCSTORAGE
=cut
	return redfishAllStorageAdapter($setExitCode, $searchNotifies, $verbose) 
	    if ($optServiceType and $optServiceType =~ m/^ired/i);
 ...
 ####
  sub connectionTest {
	my $checknext = 1;
	$exitCode = 3;
	my $save_optServiceType = $optServiceType;
	...
	if ($checknext and $main::verboseTable==100) {
	...
	if ($checknext) {
		...
		or ($optServiceType =~ m/report/i));
		#or ($optServiceType =~ m/report/i and $optChkIdentify));
	    $checknext = 0 if ($exitCode == 1); # iRMC FW REDFISH
	}
	if ($save_optServiceType and $save_optServiceType =~ m/ired/i and !$hasiRMCS5Redfish and !$msg) {
	    	addMessage("m","- ") if (!$msg);
		addMessage("m","[ERROR] Detected iRMC - but no Redfish service available");
		addExitCode(2);
		return;
	}
	if ($checknext) {
  ...
  sub getSerialID {
	...
	$serverID = iRMCReportSerialID() if ((!$optServiceType and !$serverID) or $optServiceType =~ m/^R/i);
	$serverID = redfishSerialID() if ((!$optServiceType and !$serverID) or $optServiceType =~ m/^ired/i);
  ...
  sub getOverallStatusValues {
	...
	return redfishOverallStatusValues()	if ($optServiceType and $optServiceType =~ m/^ired/i);
  ...
  sub getSystemInventoryInfo {
	...
	redfishSystemInventoryInfo() if ($optServiceType and $optServiceType =~ m/^ired/i);
  ...
  sub getSystemNotifyInformation {
	...
	redfishSystemNotifyInformation() if (!$optServiceType or $optServiceType =~ m/^ired/i);
  ...
  sub getStorageTree {
      my $out = undef;
      $out = agentGetStorageTree() if (!$optServiceType or $optServiceType =~ m/^A/i);
      print ($out) if ($out);
  } # getStorageTree
#########################################################################
sub processData {
	...
	$optDefaultTimeout = 299;
	if (!$optTimeout and $optServiceType and $optServiceType =~ m/ired/i) {
	    alarm(300);
	} elsif (!$optTimeout and $optServiceType and $optServiceType =~ m/^R/i) {
	    alarm(300);
	} elsif (!$optTimeout and $optServiceType and $optServiceType =~ m/^A/i) {
	    alarm(300);
	}
	getStorageTree() if ($optGetStorageTree);
	getAllCheckData() if (!$optChkIdentify and !$optGetStorageTree);
} # processData
...
local $SIG{ALRM} = sub {
	print 'UNKNOWN: Timeout' . "\n";
        ...
        close $psp if ($psp);
        my $curlpid = $1 if ($cid && $cid =~ m/^(\d+)[^\n]*curl$/m);
	if ($curlpid) {
	    open ( my $psp, '-|', "sh - c \"kill -9 $curlpid\" >/dev/null 2>&1");
	    my $cid = undef;
	    while (<$psp>) {
		    $cid .= $_;
	    }
...
$stateString = '' if ($optGetStorageTree);
finalize(
	$exitCode, 
	$stateString, 
	$msg,
