...
###############################################################################
  ...
  sub checkESXiRAID { # for AgentInfo
	...
	#### Controller
	{
	    @classInstances = cimEnumerateClass("SVS_PGYHostRaidController", $cmdParams);
	}
  ...
  sub getAgentsVersion {
	...
	checkESXiRAID($cmdParams) if ($isESXi);
	my @classInstances = ();
	if (!$isiRMC) {
	    {   
...
###############################################################################
  sub typeTest {
	...
	# ... since 3.40 supress iRMC
	if (!$exitCode and $optTypeTest and !$isiRMC) {
