...
This might be the address of iRMC as an example - this is deprecated for iRMCs.
...
beside "--chkidentify". 
Use "E" for ESXi, "L" for Linux, "W" for Windows.
...
=cut
  ...
  sub esxiBatteryStatusString {
	my $state = shift;
	my @batteryText = ( "Normal",
	    "Warning", "Charging", "Discharging", "Pack not installed","Failed",
	    "..undefined..",
	    );
	return undef if (!defined $state or $state !~ m/\d+/);
	return $batteryText[6] if ($state < 0 or $state > 5);
	return $batteryText[$state];
  } # esxiBatteryStatusString

  sub esxiDeviceInterface {
	my $nr = shift;
	my @text = ( "other", 
	    "SCSI", "IDE", "ieee1394", "SATA", "SAS", 
	    "FC", "..undefined..",
	);
	return undef if (!defined $nr or $nr !~ m/\d+/);
	return $text[7] if ($nr < 0 or $nr > 6);
	return $text[$nr];
  } # esxiDeviceInterface
  sub esxiReadMode {
	my $nr = shift;
	my @text = ( "..undefined..", 
	    "unknown", "readAhead", "noReadAhead", "adaptive", "notAvailable", 
	    "..undefined..",
	);
	return undef if (!defined $nr or $nr !~ m/\d+/);
	return $text[6] if ($nr < 0 or $nr > 5);
	return $text[$nr];
  } # esxiReadMode
  sub esxiWriteMode {
	my $nr = shift;
	my @text = ( "..undefined..", 
	    "unknown", "writeBack", "writeThrough","adaptive", "writeBackBadBBU", 
	    "readOnly","readWrite", "alwaysReadWrite","notAvailable", "..undefined..",
	);
	return undef if (!defined $nr or $nr !~ m/\d+/);
	return $text[10] if ($nr < 0 or $nr > 9);
	return $text[$nr];
  } # esxiWriteMode
...
#########################################################################
  ...
  sub cimWbemEnumerateClass {
	...
	foreach my $singleClass (@PullItems) {
		my @tagArray = ();
		next if (!$singleClass);
		#print ">>>$singleClass<<<\n" if ($singleClass and $singleClass =~ m/PreRequisitesText/);
		while ($singleClass) { # split for CIM elements
		    # search next tag
		    my $tag = undef;
		    my $stripstring = 0;
		    if ($singleClass and $singleClass =~ m/^\-PreRequisitesText=/) {
			$singleClass =~ m/^\-([a-z,A-Z]+=\"[^\"]*\")/m;
			$tag = $1;
			#print "found PreRequisitesText $tag\n" if ($tag);
			$stripstring = 1 if ($tag);
		    }
		    if (!$tag) {
			$singleClass =~ m/^\-(.*)\n/;
			$tag = $1;
			...
			if (!$stripstring) {
			    ...
			    $singleClass =~ s/^\-[a-z,A-Z]+=\"[^\"]*\"\n//m;
			}
		    }
		    #print "+++$singleClass+++\n" if ($singleClass and $stripstring);
...
#########################################################################
  ...
  sub getAllMemoryModules {
	...
	my @configText = ( "none",
		...
		"raid", "notUsable", "configurationError", "..unexpected..",
	);
	...
	foreach my $refClass (@classInstances) {
		...
		my $memconfig = $oneClass{"MemModuleConfiguration"}; #
		...
		$memconfig = undef if (defined $memconfig and $memconfig eq '');
		$memconfig = 9 if ($memconfig and $memconfig > 8);
		my $memconfigtext = undef;
		$memconfigtext = $configText[$memconfig] if ($memconfig > 0);
		...
		if ($verbose) {
			...
			addKeyLongValue("v","Configuration", $memconfigtext);
			$variableVerboseMessage .= "\n";
		} elsif ($searchNotifies and ($localState == 1 or $localState == 2)) {
			...
			addKeyLongValue("l","Configuration", $memconfigtext);
			$longMessage .= "\n";
		}
	} # foreach class
	addExitCode($tmpExitCode) if ($setExitCode);
  ...
  our $hasESXiRAID = 0;
  sub checkESXiRAID { # for AgentInfo
	return if (!$optAgentInfo);
	my @classInstances = ();
	#### Controller
	{
	    @classInstances = cimEnumerateClass("SVS_PGYHostRaidController");
	    cimPrintClass(\@classInstances, "SVS_PGYHostRaidController");
	}
	return if ($#classInstances < 0);   # not available
	$hasESXiRAID = 1;
  } # checkESXiRAID
  sub getESXiRAIDController {
	my $searchNotifies = shift;
	my $verbose = shift;
	return if (!$searchNotifies and !$verbose);
	my @classInstances = ();
	#### Controller
	{
	    @classInstances = cimEnumerateClass("SVS_PGYHostRaidController");
	...
	foreach my $refClass (@classInstances) {
	    next if !$refClass;
		my %oneClass = %{$refClass};
		my $ename = $oneClass{"ElementName"};
		my $name = $oneClass{"Name"};
		my $vendor = $oneClass{"Vendor"};
		my $sasaddress = $oneClass{"SASAddress"};
		my $statusText = $oneClass{"Status"};
		my $status = $oneClass{"HealthState"};
		my $memory = $oneClass{"MemorySize"};
		$medium = undef;
		$medium = "v" if ($verbose);
		$medium = "l" if ($searchNotifies and !$verbose and $status != 5);
		my $memoMB = 0;
		$memoMB = $memory / 1024 / 1024 if ($memory and $memory > (1024*1024));
		if ($medium) {
			addStatusTopic($medium,$statusText,"RaidController",$name);
			addName($medium,$ename);
			addKeyLongValue($medium,"Vendor",$vendor);
			addKeyValue($medium,"SASAddress",$sasaddress);
			if ($memory) {
			    addKeyMB($medium,"MemorySize", $memoMB) if ($memoMB);
			    addKeyIntValueUnit($medium,"MemorySize",$memory,"B") if (!$memoMB);
			}
			addMessage($medium,"\n");
		}
	}
	#### Ports
	{
	    @classInstances = cimEnumerateClass("SVS_PGYPortController");
	    cimPrintClass(\@classInstances, "SVS_PGYPortController");
	}
	return if ($#classInstances < 0);   # not available
	$medium = undef;
	addTableHeader("v","Raid Port Controller") if ($verbose);
	foreach my $refClass (@classInstances) {
	    next if !$refClass;
		my %oneClass = %{$refClass};
		my $ename = $oneClass{"ElementName"}; # same like controller above
		# Same like "Name"
		my $model = $oneClass{"Model"};
		my $protocol = $oneClass{"ProtocolDescription"};
		my $deviceID = $oneClass{"DeviceID"};
		my $statusText = $oneClass{"Status"};
		my $status = $oneClass{"HealthState"};
		my $battery = $oneClass{"BatteryStatus"};
		# counter
		my $nrLogicalDevices = $oneClass{"NumberLogicalDevices"};
		my $nrPhysDisks = $oneClass{"NumberPhysDisks"};
		# localization:
		my $pciSegmentNumber	= $oneClass{"PCISegmentNumber"}; # missing due to LSI infos (Dec 2016)
		my $pciBusNumber	= $oneClass{"PCIBusNumber"};
		my $pciDeviceNumber	= $oneClass{"PCIDeviceNumber"};
		my $pciFunctionNumber	= $oneClass{"PCIFunctionNumber"};
		# vendor:
		my $pciVendorID		= $oneClass{"PCIVendorID"};
		my $pciSubVendorID	= $oneClass{"PCISubVendorID"};
		my $pciDeviceID		= $oneClass{"PCIDeviceID"};
		my $pciSubDeviceID	= $oneClass{"PCISubDeviceID"};
		...
		my $pciID = undef;
		my $pciconcat = undef;
		if (defined $pciBusNumber and defined $pciDeviceNumber and defined $pciFunctionNumber) {
		    $pciID = $pciBusNumber;
		    $pciID .= "-";
		    $pciID .= $pciDeviceNumber if (defined $pciDeviceNumber);
		    $pciID .= "-";
		    $pciID .= $pciFunctionNumber if (defined $pciFunctionNumber);
		    $pciconcat = $pciID; # missing "segment number ?"
		} else {
		    $pciID = $deviceID;
		}
		my $pcivendor = "$pciVendorID,$pciSubVendorID,$pciDeviceID,$pciSubDeviceID";
		my $batteryText = esxiBatteryStatusString($battery);
		if ($medium) {
			addStatusTopic($medium,$statusText,"RaidPortController",$pciID);
			addName($medium,$ename);
			addProductModel($medium,undef,$model);
			addKeyValue($medium,"Protocol",$protocol);
			#addKeyValue($medium,"DeviceID",$deviceID);
			addKeyLongValue($medium,"BatteryStatus",$batteryText);
			addKeyIntValue($medium,"LogicalDevices", $nrLogicalDevices);
			addKeyIntValue($medium,"PhysDisks", $nrPhysDisks);
			addKeyValue($medium,"PCI-Bus-Dev-Fct",$pciconcat);
			#addKeyValue($medium,"PCI-Seg-Bus-Dev-Fct",$pciconcat);
			addKeyLongValue($medium,"PCI-VendorInformation", $pcivendor);
			addMessage($medium,"\n");
		}	
	}
  } # getESXiRAIDController
  sub getESXiRAIDPhysical {
	my $searchNotifies = shift;
	my $verbose = shift;
	my $medium = undef;
	return if (!$searchNotifies and !$verbose);
	my @classInstances = ();
	my $diskcontroller = undef;
	my $diskslot = undef;
	# SVS_PGYControllerDisk - ATTENTION: not unique assignment by serial number
	    {
		@classInstances = cimEnumerateClass("SVS_PGYControllerDisk");
		cimPrintClass(\@classInstances, "SVS_PGYControllerDisk");
	    }
	    if ($#classInstances >= 0) {
		foreach my $refClass (@classInstances) {
		    next if !$refClass;
		    my %oneClass = %{$refClass};
		    my $controllerHint = $oneClass{"GroupComponent"}; 
		    my $diskHint = $oneClass{"PartComponent"}; 
		    my $cont = undef;
		    my $disk = undef;
		    $controllerHint =~ m/Name=\"([^\"]+)\"/;
		    $cont = $1 if ($1);
		    $diskHint =~ m/DeviceID=\"([^\"]+)\"/;
		    $disk = $1 if ($1);
		    if ($diskcontroller->{$disk}) {
			$diskcontroller->{$disk} .= ",$cont" if ($disk and $cont);
		    } else {
			$diskcontroller->{$disk} = $cont if ($disk and $cont);
		    }
		} # for
	...
	# SVS_PGYDiskDrive
	    {
		@classInstances = cimEnumerateClass("SVS_PGYDiskDrive");
		cimPrintClass(\@classInstances, "SVS_PGYDiskDrive");
	    }
	    if ($#classInstances >= 0) {
		# ATTENTION: prevent double prints
		my %printedDeviceID = ();
		addTableHeader("v","Raid Physical Disk Drive") if ($verbose);
		foreach my $refClass (@classInstances) {
		    next if !$refClass;
		    my %oneClass = %{$refClass};
		    my $serial = $oneClass{"DeviceID"};
		    next if ($printedDeviceID{$serial});
		    $printedDeviceID{$serial} = 1;
		    my $ename = $oneClass{"ElementName"}; 
		    my $status = $oneClass{"HealthState"};
		    my $statusText = $oneClass{"Status"};
		    my $sasaddress = $oneClass{"SASAddress"};
		    my $deviceInterface = $oneClass{"DeviceInterface"};
		    my $uncompressedDataRate = $oneClass{"UncompressedDataRate"};
		    # UncompressedDataRate kB -> MB
		    my $predictiveFailureCount	= $oneClass{"PredictiveFailureCount"};
		    my $nrOfBadBlocks		= $oneClass{"NumberOfBadBlocks"};
		    my $nrOfErrors		= $oneClass{"NumberOfErrors"};
		    my $SSDPowerOnHours		    = $oneClass{"SSDPowerOnHours"};
		    my $SSDRatedWriteEndurance	    = $oneClass{"SSDRatedWriteEndurance"}; #%
		    my $SSDRemainingReserveSpace    = $oneClass{"SSDRemainingReserveSpace"}; #B
		    my $SSDTotalDataWritten	    = $oneClass{"SSDTotalDataWritten"}; #B
		    my $SSDEstimatedLifetime	    = $oneClass{"SSDEstimatedLifetime"};
		    my $device = esxiDeviceInterface($deviceInterface);
		    my $rateMB = 0;
		    $rateMB = $uncompressedDataRate / 1000 if ($uncompressedDataRate and $uncompressedDataRate > 1000);
		    my $cntrl = $diskcontroller->{$serial};
		    $medium = undef;
		    $medium = "v" if ($verbose);
		    $medium = "l" if ($searchNotifies and !$verbose and $status != 5);
		    if ($medium) {
			addStatusTopic($medium,$statusText,"DiskDrive",'');
			addName($medium,$ename);
			addSerialIDs($medium,$serial);
			addKeyLongValue($medium,"Controller", $cntrl);
			addKeyValue($medium,"SASAddress",$sasaddress) if ($verbose >= 3);
			addKeyValue($medium,"DeviceInterface",$device);
			addKeyMB($medium,"DataRate",$rateMB);
			addKeyIntValue($medium,"PredictiveFailureCount", $predictiveFailureCount) if ($predictiveFailureCount);
			addKeyIntValue($medium,"NumberOfBadBlocks", $nrOfBadBlocks) if ($nrOfBadBlocks);
			addKeyIntValue($medium,"NumberOfErrors", $nrOfErrors) if ($nrOfErrors);
			addKeyValueUnit($medium,"SSDPowerOnHours", $SSDPowerOnHours, "h");
			addKeyValueUnit($medium,"SSDRatedWriteEndurance", $SSDRatedWriteEndurance, "%");
			addKeyValueUnit($medium,"SSDRemainingReserveSpace", $SSDRemainingReserveSpace, "B");
			addKeyValueUnit($medium,"SSDTotalDataWritten", $SSDTotalDataWritten, "B");
			addKeyValue($medium,"SSDEstimatedLifetime", $SSDEstimatedLifetime);
			addMessage($medium,"\n");
		    }
		} # for
	    } 
	# SVS_PGYDiskExtent
	    {
		@classInstances = cimEnumerateClass("SVS_PGYDiskExtent");
		cimPrintClass(\@classInstances, "SVS_PGYDiskExtent");
	    }
	    if ($#classInstances >= 0) {
		addTableHeader("v","Raid Physical Disk Extend") if ($verbose);
		my %printedDeviceID = ();
		foreach my $refClass (@classInstances) {
		    next if !$refClass;
		    my %oneClass = %{$refClass};
		    ...
		    my $status = $oneClass{"HealthState"};
		    my $statusText = $oneClass{"Status"};
		    my $NumberOfBlocks = $oneClass{"NumberOfBlocks"};
		    my $ConsumableBlocks = $oneClass{"ConsumableBlocks"};
		    my $used = $oneClass{"ConfiguredDisk"};
		    my $foreign = $oneClass{"ForeignConfig"};
		    $medium = undef;
		    $medium = "v" if ($verbose);
		    $medium = "l" if ($searchNotifies and !$verbose and $status != 5);
		    if ($medium) {
			addStatusTopic($medium,$statusText,"DiskDriveExtend",'');
			addName($medium,$ename);
			addSerialIDs($medium,$serial);
			addKeyValue($medium,"ConfiguredForLogic", $used);
			addKeyValue($medium,"ForeignConfig", $foreign) if ($foreign and $foreign=~m/true/i);
			addKeyIntValue($medium,"NumberOfBlocks",$NumberOfBlocks);
			addKeyIntValue($medium,"ConsumableBlocks",$ConsumableBlocks);
			addMessage($medium,"\n");
		    }
		} # for
	    } 
	# SVS_PGYPhysicalDiskPackage
	    {
		@classInstances = cimEnumerateClass("SVS_PGYPhysicalDiskPackage");
		cimPrintClass(\@classInstances, "SVS_PGYPhysicalDiskPackage");
	    }
	    if ($#classInstances >= 0) {
		addTableHeader("v","Raid Physical Disk Package") if ($verbose);
		my %printedDeviceID = ();
		foreach my $refClass (@classInstances) {
		    next if !$refClass;
		    my %oneClass = %{$refClass};
		    ...
		    my $version = $oneClass{"FirmwareVersion"};
		    my $powered = $oneClass{"PoweredOn"};
		    $medium = undef;
		    $medium = "v" if ($verbose);
		    $medium = "l" if ($searchNotifies and !$verbose and $status != 5);
		    if ($medium) {
			addStatusTopic($medium,$statusText,"DiskDrivePackage",'');
			addName($medium,$ename);
			addSerialIDs($medium,$serial);
			addKeyLongValue($medium,"Manufacturer", $manu);
			addProductModel($medium,undef,$model);
			addKeyValue($medium,"FirmwareVersion", $version);
			addKeyValue($medium,"PoweredOn", $powered);
			addMessage($medium,"\n");
		    }
		} # for
	    } 
	# SVS_PGYDiskInSlot - ATTENTION: not unique assignment
	    {
		@classInstances = cimEnumerateClass("SVS_PGYDiskInSlot");
		cimPrintClass(\@classInstances, "SVS_PGYDiskInSlot");
	    }
	    if ($#classInstances >= 0) {
		foreach my $refClass (@classInstances) {
		    next if !$refClass;
		    my %oneClass = %{$refClass};
		    my $packageHint = $oneClass{"Dependent"}; 
		    my $slotHint = $oneClass{"Antecedent"}; 
		    my $pkg = undef;
		    my $slot = undef;
		    $packageHint =~ m/Tag=\"([^\"]+)\"/;
		    $pkg = $1 if ($1);
		    ...
		    $slot = $1 if ($1);
		    if ($diskslot->{$slot}) {
			$diskslot->{$slot} .= ",$pkg" if ($slot and $pkg);
		    } else {
			$diskslot->{$slot} = $pkg if ($slot and $pkg);
		    }
		} # for
	    } 
	# SVS_PGYPhysicalDiskSlot
	    {
		@classInstances = cimEnumerateClass("SVS_PGYPhysicalDiskSlot");
		cimPrintClass(\@classInstances, "SVS_PGYPhysicalDiskSlot");
	    }
	    if ($#classInstances >= 0) {
		addTableHeader("v","Raid Physical Disk Slot") if ($verbose);
		my %printedDeviceID = ();
		foreach my $refClass (@classInstances) {
		    next if !$refClass;
		    my %oneClass = %{$refClass};
		    my $tag = $oneClass{"Tag"};
		    next if ($printedDeviceID{$tag});
		    $printedDeviceID{$tag} = 1;
		    my $status = $oneClass{"HealthState"};
		    my $statusText = $oneClass{"Status"};
		    $medium = undef;
		    $medium = "v" if ($verbose);
		    $medium = "l" if ($searchNotifies and !$verbose and $status != 5);
		    if ($medium) {
			addStatusTopic($medium,$statusText,"DiskDriveSlot",$tag);
			addKeyLongValue($medium,"AssignedDisks", $diskslot->{$tag});
			addMessage($medium,"\n");
		    }
		} # for
	    }
  } # getESXiRAIDPhysical
  sub getESXiRAIDLogical {
	my $searchNotifies = shift;
	my $verbose = shift;
	my $medium = undef;
	return if (!$searchNotifies and !$verbose);
	my @classInstances = ();
	# SVS_PGYStoragePoolCompositeExtent
	    {
		@classInstances = cimEnumerateClass("SVS_PGYStoragePoolCompositeExtent");
		cimPrintClass(\@classInstances, "SVS_PGYStoragePoolCompositeExtent");
	    }
	    if ($#classInstances >= 0) {
		addTableHeader("v","Raid Logical Drive Composite") if ($verbose);
		foreach my $refClass (@classInstances) {
		    next if !$refClass;
		    my %oneClass = %{$refClass};
		    my $deviceID = $oneClass{"DeviceID"};
		    my $name = $oneClass{"Name"}; # same as ElementName, 
		    my $status = $oneClass{"HealthState"};
		    my $statusText = $oneClass{"Status"};
		    my $level = $oneClass{"RaidLevel"};
		    my $IsConcatenated = $oneClass{"IsConcatenated"};
		    my $ConsumableBlocks = $oneClass{"ConsumableBlocks"}; #MB
		    my $ReadMode = $oneClass{"ReadMode"};
		    my $WriteMode = $oneClass{"WriteMode"};
		    my $rmode = esxiReadMode($ReadMode);
		    my $wmode = esxiWriteMode($WriteMode);
		    $medium = undef;
		    $medium = "v" if ($verbose);
		    $medium = "l" if ($searchNotifies and !$verbose and $status != 5);
		    if ($medium) {
			addStatusTopic($medium,$statusText,"LogicalDrive",$deviceID);
			addName($medium,$name);
			addKeyValue($medium,"Level",$level);
			addKeyValue($medium,"IsConcatenated",$IsConcatenated);
			addKeyMB($medium,"ConsumableBlocks",$ConsumableBlocks);
			addKeyValue($medium,"ReadMode",$rmode);
			addKeyValue($medium,"WriteMode",$wmode);
			addMessage($medium,"\n");
		    } 
		} # for
	    }
  } # getESXiRAIDLogical
  sub getAllESXiRAIDStorageAdapter {
      my $searchNotifies = shift;
      my $verbose = shift;
      return if (!$searchNotifies and !$verbose);
      getESXiRAIDController($searchNotifies,$verbose);
      if ($hasESXiRAID) {
	  getESXiRAIDPhysical($searchNotifies,$verbose);
	  getESXiRAIDLogical($searchNotifies,$verbose);
      }
  } # getAllESXiRAIDStorageAdapter
  sub getAllStorageAdapter {
	...
	if ($isESXi) {
	    getAllESXiRAIDStorageAdapter($searchNotifies,$verbose);
	}
  ...
  sub getSystemInventoryInfo {
	...
	if ($#classInstances >= 0) {
		...
		if (!$optAgentInfo) {
		    ...
		    addKeyValue("l", "RAIDProvider","on") if ($hasESXiRAID);
		    addMessage("l","\n");
		}
	    }
...
sub processData {
	...
	if ($exitCode != 2 and $optChkClass) {
		...
		if ($isiRMC) {
		    addMessage("m", "- deprecated iRMC CIM support");
		    $exitCode = 3;
		} else {
		    if (!$isESXi and !$isLINUX and !$isWINDOWS
		    and $main::verbose <= 10) 
		    {
			$exitCode = 3;
			$msg .= "- ";
...
local $SIG{ALRM} = sub {
	...
	if ($wsmanpid) {
			open ( my $psp, '-|', "sh - c \"kill -9 $wsmanpid\" >/dev/null 2>&1");
			my $cid = undef;
			while (<$psp>) {
				$cid .= $_;
			}
