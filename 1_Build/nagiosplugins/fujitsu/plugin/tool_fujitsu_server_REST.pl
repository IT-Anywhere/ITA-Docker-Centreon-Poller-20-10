...
###############################################################################
  ...
  sub oneConnectionTest {
	...
	if (defined $rc and $rc == 0) {
		...
		#$optServiceMode = "iredwait" if ($name and $name =~ m/iRMC Redfish .*Uninitialized.*/);
		#$optServiceMode = "ired" if ($name and $name =~ m/iRMC Redfish/);
	    }
	    if ($result =~ m/Redfish\-Service=\"([^\"]*)\"/) {
		my $name = $1;
		$optServiceMode = "iredwait" if (!$optServiceMode and $name and $name =~ m/iRMC Redfish .*Uninitialized.*/);
		$optServiceMode = "iRed" if ($name and $name =~ m/iRMC Redfish$/);
	    }
	}
  ...
  sub connectionTest {
	...
	if ($usableCheck) {
		...
		$prot .= "-iRMC-Redfish" if ($optServiceMode and $optServiceMode =~ m/^ired/i);
		$prot .= "-Server-Control" if ($optServiceMode and $optServiceMode =~ m/^A/i);
		#$prot .= "-iRMC-eLCM" if ($optServiceMode and $optServiceMode =~ m/^ei/i);
	    }
	    if ($main::processPrint and defined $rc) {
		    print "<<< ";
		    print "OK" if (!$rc);
